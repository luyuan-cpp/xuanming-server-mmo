...
//    generated/tables/mission.json 不是 protojson,是导表器自己 json.dumps 出来的,
...
#include "CfgMissionRow.generated.h"

/// Mission 的一行。字段名与 mission.json 的键逐字符一致(snake_case),
/// 见文件头「字段名口径」。
///
...
/// 这个代价不再是静默的:MissionTable.cpp 在装载路径上对**由无符号收窄而来**的列
/// 做值域检查,越界会打 Error 并点名表/行/列。
USTRUCT(BlueprintType)
struct FCfgMissionRow : public FTableRowBase
{
	GENERATED_BODY()

	UPROPERTY(BlueprintReadOnly, Category = "Config|Mission")
	int32 id = 0;

	/** condition表的id */
	UPROPERTY(BlueprintReadOnly, Category = "Config|Mission")
	TArray<int32> condition_id;

	/** 1：主线，2：活动 */
	UPROPERTY(BlueprintReadOnly, Category = "Config|Mission")
	int32 mission_type = 0;

	/** 自定义：非特殊情况1 */
	UPROPERTY(BlueprintReadOnly, Category = "Config|Mission")
	int32 mission_sub_type = 0;

	/** 0并行1顺序 */
	UPROPERTY(BlueprintReadOnly, Category = "Config|Mission")
	int32 condition_order = 0;
	...
	UPROPERTY(BlueprintReadOnly, Category = "Config|Mission")
	TArray<int32> next_mission_id;

	/** 每个condition对应的目标数量(覆盖condition表默认值) */
	UPROPERTY(BlueprintReadOnly, Category = "Config|Mission")
	TArray<int32> target_count;
};
