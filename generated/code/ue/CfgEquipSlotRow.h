...
//    generated/tables/equipslot.json 不是 protojson,是导表器自己 json.dumps 出来的,
...
#include "CfgEquipSlotRow.generated.h"

/// EquipSlot 的一行。字段名与 equipslot.json 的键逐字符一致(snake_case),
/// 见文件头「字段名口径」。
///
...
/// 这个代价不再是静默的:EquipSlotTable.cpp 在装载路径上对**由无符号收窄而来**的列
/// 做值域检查,越界会打 Error 并点名表/行/列。
USTRUCT(BlueprintType)
struct FCfgEquipSlotRow : public FTableRowBase
{
	GENERATED_BODY()

	/** 装备栏槽位号(对应 BagAllData.ItemEntry.pos) */
	UPROPERTY(BlueprintReadOnly, Category = "Config|EquipSlot")
	int32 id = 0;

	/** 该槽位接受的装备部位;没在本表里出现的槽位不接受任何东西 */
	UPROPERTY(BlueprintReadOnly, Category = "Config|EquipSlot")
	int32 equip_kind = 0;
};
