...
//    generated/tables/attributepool.json 不是 protojson,是导表器自己 json.dumps 出来的,
...
#include "CfgAttributePoolRow.generated.h"

/// AttributePool 的一行。字段名与 attributepool.json 的键逐字符一致(snake_case),
/// 见文件头「字段名口径」。
///
...
/// 这个代价不再是静默的:AttributePoolTable.cpp 在装载路径上对**由无符号收窄而来**的列
/// 做值域检查,越界会打 Error 并点名表/行/列。
USTRUCT(BlueprintType)
struct FCfgAttributePoolRow : public FTableRowBase
{
	GENERATED_BODY()
	...
	/** 单维度可分配上限(0=不限) */
	UPROPERTY(BlueprintReadOnly, Category = "Config|AttributePool")
	int32 dimension_cap = 0;

	/** 重置(洗点)金币 */
	UPROPERTY(BlueprintReadOnly, Category = "Config|AttributePool")
	int64 reset_cost_gold = 0;

	/** 低于此等级重置免费(0=不免费) */
	UPROPERTY(BlueprintReadOnly, Category = "Config|AttributePool")
	int32 reset_free_below_level = 0;
