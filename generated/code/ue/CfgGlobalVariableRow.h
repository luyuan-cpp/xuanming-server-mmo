...
//    generated/tables/globalvariable.json 不是 protojson,是导表器自己 json.dumps 出来的,
...
#include "CfgGlobalVariableRow.generated.h"

/// GlobalVariable 的一行。字段名与 globalvariable.json 的键逐字符一致(snake_case),
/// 见文件头「字段名口径」。
///
...
/// 这个代价不再是静默的:GlobalVariableTable.cpp 在装载路径上对**由无符号收窄而来**的列
/// 做值域检查,越界会打 Error 并点名表/行/列。
USTRUCT(BlueprintType)
struct FCfgGlobalVariableRow : public FTableRowBase
{
	GENERATED_BODY()

	UPROPERTY(BlueprintReadOnly, Category = "Config|GlobalVariable")
	int32 id = 0;
};
