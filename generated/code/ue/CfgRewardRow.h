...
//    generated/tables/reward.json 不是 protojson,是导表器自己 json.dumps 出来的,
...
#include "CfgRewardRow.generated.h"

/// Reward.reward 的子消息行。
USTRUCT(BlueprintType)
struct FCfgRewardReward
{
	GENERATED_BODY()

	UPROPERTY(BlueprintReadOnly, Category = "Config|Reward")
	int32 reward_item = 0;

	UPROPERTY(BlueprintReadOnly, Category = "Config|Reward")
	int32 reward_count = 0;
};

/// Reward 的一行。字段名与 reward.json 的键逐字符一致(snake_case),
/// 见文件头「字段名口径」。
///
...
/// 这个代价不再是静默的:RewardTable.cpp 在装载路径上对**由无符号收窄而来**的列
/// 做值域检查,越界会打 Error 并点名表/行/列。
USTRUCT(BlueprintType)
struct FCfgRewardRow : public FTableRowBase
{
	GENERATED_BODY()
	...
	UPROPERTY(BlueprintReadOnly, Category = "Config|Reward")
	TArray<FCfgRewardReward> reward;
};
