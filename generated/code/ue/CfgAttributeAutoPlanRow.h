...
//    generated/tables/attributeautoplan.json 不是 protojson,是导表器自己 json.dumps 出来的,
...
#include "CfgAttributeAutoPlanRow.generated.h"

/// AttributeAutoPlan 的一行。字段名与 attributeautoplan.json 的键逐字符一致(snake_case),
/// 见文件头「字段名口径」。
///
...
/// 这个代价不再是静默的:AttributeAutoPlanTable.cpp 在装载路径上对**由无符号收窄而来**的列
/// 做值域检查,越界会打 Error 并点名表/行/列。
USTRUCT(BlueprintType)
struct FCfgAttributeAutoPlanRow : public FTableRowBase
{
	GENERATED_BODY()

	/** 方案 id */
	UPROPERTY(BlueprintReadOnly, Category = "Config|AttributeAutoPlan")
	int32 id = 0;
