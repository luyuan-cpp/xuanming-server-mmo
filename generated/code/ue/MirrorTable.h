...
// ---------------------------------------------------------------------------
//
//  模块依赖:本头要 Core + CoreUObject;配套的 MirrorTable.cpp 还要 Json +
...
struct FCfgMirrorSnapshot
{
	/// 全部行,按 mirror.json 里的出现顺序。
	TArray<FCfgMirrorRow> Rows;
	/// id -> 行下标。
	TMap<int32, int32> IdIndex;
	/// scene_id -> 全部命中行的下标(二级索引)。
	TMap<int32, TArray<int32>> SceneIdIndex;
	/// main_scene_id -> 全部命中行的下标(二级索引)。
	TMap<int32, TArray<int32>> MainSceneIdIndex;
};
 ...
 * ---- 生命周期契约(热更)----
 *
 * Load*() 会**整批**建好一个新的 FCfgMirrorSnapshot 再原子替换成员指针;旧快照
 * 在最后一个持有者放手时析构。所以下面所有返回 `const FCfgMirrorRow*` /
 * `const TArray<...>&` / `const FCfgMirrorSnapshot&` 的接口,
 * **返回值只在下一次 Load 之前有效**:
 *
 *     ✅ 存 id,用的时候现查
 *     ❌ 把 const FCfgMirrorRow* 存进成员、容器、Lambda 捕获、Latent 节点
 *     ❌ 把**行下标**(int32)存起来 —— 下标只在取它的那一代快照里有意义。
 *        它可平凡拷贝,不会像野指针那样崩;换代之后是**静默读到另一行**,更难查。
 *
 * UE 的 GC **救不了你**:FCfgMirrorRow 是 USTRUCT 不是 UObject,它躺在一个
 ...
 *
 *     TSharedPtr<const FCfgMirrorSnapshot> Pinned = Table->PinSnapshot();
 *     const FCfgMirrorRow* Row = Pinned->Rows.IsValidIndex(I) ? &Pinned->Rows[I] : nullptr;
 *
 * ---- 线程契约 ----
 *
...
class MMORPGCONFIG_API UMirrorTable : public UObject
{
	GENERATED_BODY()

public:
	using FSnapshot = FCfgMirrorSnapshot;

	/// 这张表在 generated/tables 下的文件名(全小写,与 .pb 同口径)。
	static const TCHAR* FileName() { return TEXT("mirror.json"); }
	...
	const TArray<FCfgMirrorRow>& GetRows() const { return Snapshot->Rows; }

	/// 按行下标取行;越界返回 nullptr。
	const FCfgMirrorRow* RowAt(int32 RowIndex) const
	{
	...
	const FCfgMirrorRow* FindById(int32 Id) const;

	/// 命中不到返回 nullptr,不打日志(「查不到是正常分支」的场景用这个)。
	const FCfgMirrorRow* FindByIdSilent(int32 Id) const;

	UFUNCTION(BlueprintCallable, Category = "Config|Mirror", DisplayName = "Find By Id")
	bool K2_FindById(int32 Id, FCfgMirrorRow& OutRow) const;

	UFUNCTION(BlueprintPure, Category = "Config|Mirror")
	...
	TArray<const FCfgMirrorRow*> FindByIds(const TArray<int32>& Ids) const;

	/** 随机取一行。表为空返回 nullptr。用 FMath::RandRange,不引 <random>。 */
	const FCfgMirrorRow* RandOne() const;
	...
	/// scene_id == Key 的全部行。
	TArray<const FCfgMirrorRow*> GetBySceneId(int32 Key) const;
	/// 零分配版:下标数组,配 RowAt() 用。
	...
	/// main_scene_id == Key 的全部行。
	TArray<const FCfgMirrorRow*> GetByMainSceneId(int32 Key) const;
	/// 零分配版:下标数组,配 RowAt() 用。
	const TArray<int32>& GetMainSceneIdIndices(int32 Key) const;
	int32 CountByMainSceneIdIndex(int32 Key) const;
	const TMap<int32, TArray<int32>>& GetMainSceneIdIndex() const
	{
		return Snapshot->MainSceneIdIndex;
	}
	...
	TArray<const FCfgMirrorRow*> Where(TFunctionRef<bool(const FCfgMirrorRow&)> Pred) const;
	const FCfgMirrorRow* First(TFunctionRef<bool(const FCfgMirrorRow&)> Pred) const;
