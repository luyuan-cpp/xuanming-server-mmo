...
// ---------------------------------------------------------------------------
//
//  模块依赖:本头要 Core + CoreUObject;配套的 AttributePoolTable.cpp 还要 Json +
...
#include "CfgAttributePoolRow.h"

#include "AttributePoolTable.generated.h"

/// AttributePool 的一代数据 + 全部索引。
///
/// 索引里存的是**行下标**而不是行指针:建索引时 Rows 还在增长,存指针会在
/// TArray 扩容那一刻全部失效。行下标在快照内永远有效。
struct FCfgAttributePoolSnapshot
{
	/// 全部行,按 attributepool.json 里的出现顺序。
...
/**
 * AttributePool 配置表管理器。
 *
 * ---- 生命周期契约(热更)----
 *
 * Load*() 会**整批**建好一个新的 FCfgAttributePoolSnapshot 再原子替换成员指针;旧快照
 * 在最后一个持有者放手时析构。所以下面所有返回 `const FCfgAttributePoolRow*` /
 * `const TArray<...>&` / `const FCfgAttributePoolSnapshot&` 的接口,
 * **返回值只在下一次 Load 之前有效**:
 *
 *     ✅ 存 id,用的时候现查
 *     ❌ 把 const FCfgAttributePoolRow* 存进成员、容器、Lambda 捕获、Latent 节点
 *     ❌ 把**行下标**(int32)存起来 —— 下标只在取它的那一代快照里有意义。
 *        它可平凡拷贝,不会像野指针那样崩;换代之后是**静默读到另一行**,更难查。
 *
 * UE 的 GC **救不了你**:FCfgAttributePoolRow 是 USTRUCT 不是 UObject,它躺在一个
 ...
 *
 *     TSharedPtr<const FCfgAttributePoolSnapshot> Pinned = Table->PinSnapshot();
 *     const FCfgAttributePoolRow* Row = Pinned->Rows.IsValidIndex(I) ? &Pinned->Rows[I] : nullptr;
 *
 * ---- 线程契约 ----
 *
...
class MMORPGCONFIG_API UAttributePoolTable : public UObject
{
	GENERATED_BODY()

public:
	using FSnapshot = FCfgAttributePoolSnapshot;

	/// 这张表在 generated/tables 下的文件名(全小写,与 .pb 同口径)。
	static const TCHAR* FileName() { return TEXT("attributepool.json"); }
	...
	/// 从目录装载 <ConfigDir>/attributepool.json。
	...
	const TArray<FCfgAttributePoolRow>& GetRows() const { return Snapshot->Rows; }

	/// 按行下标取行;越界返回 nullptr。
	const FCfgAttributePoolRow* RowAt(int32 RowIndex) const
	{
	...
	const FCfgAttributePoolRow* FindById(int32 Id) const;

	/// 命中不到返回 nullptr,不打日志(「查不到是正常分支」的场景用这个)。
	const FCfgAttributePoolRow* FindByIdSilent(int32 Id) const;

	UFUNCTION(BlueprintCallable, Category = "Config|AttributePool", DisplayName = "Find By Id")
	bool K2_FindById(int32 Id, FCfgAttributePoolRow& OutRow) const;

	UFUNCTION(BlueprintPure, Category = "Config|AttributePool")
	...
	TArray<const FCfgAttributePoolRow*> FindByIds(const TArray<int32>& Ids) const;

	/** 随机取一行。表为空返回 nullptr。用 FMath::RandRange,不引 <random>。 */
	const FCfgAttributePoolRow* RandOne() const;
	...
	TArray<const FCfgAttributePoolRow*> Where(TFunctionRef<bool(const FCfgAttributePoolRow&)> Pred) const;
	const FCfgAttributePoolRow* First(TFunctionRef<bool(const FCfgAttributePoolRow&)> Pred) const;
