...
//    generated/tables/guildshop.json 不是 protojson,是导表器自己 json.dumps 出来的,
...
#include "CfgGuildShopRow.generated.h"

/// GuildShop 的一行。字段名与 guildshop.json 的键逐字符一致(snake_case),
/// 见文件头「字段名口径」。
///
...
/// 这个代价不再是静默的:GuildShopTable.cpp 在装载路径上对**由无符号收窄而来**的列
/// 做值域检查,越界会打 Error 并点名表/行/列。
USTRUCT(BlueprintType)
struct FCfgGuildShopRow : public FTableRowBase
{
	GENERATED_BODY()

	/** 商品 id */
	UPROPERTY(BlueprintReadOnly, Category = "Config|GuildShop")
	int32 id = 0;
	...
	/** 每份消耗帮贡(1..1000000000) 上限保证"单价 × 单次最多份数"不溢出 */
	UPROPERTY(BlueprintReadOnly, Category = "Config|GuildShop")
	int64 cost_contribution = 0;

	/** 需要的帮会等级(1..GuildLevel 最高级) */
	UPROPERTY(BlueprintReadOnly, Category = "Config|GuildShop")
	int32 required_guild_level = 0;

	/** 限购周期:0 不限 / 1 每游戏日 / 2 每周(周一 05:00 切周) */
	UPROPERTY(BlueprintReadOnly, Category = "Config|GuildShop")
	int32 limit_period = 0;
