...
//    generated/tables/pet.json 不是 protojson,是导表器自己 json.dumps 出来的,
...
#include "CfgPetRow.generated.h"

/// Pet 的一行。字段名与 pet.json 的键逐字符一致(snake_case),
/// 见文件头「字段名口径」。
///
...
/// 这个代价不再是静默的:PetTable.cpp 在装载路径上对**由无符号收窄而来**的列
/// 做值域检查,越界会打 Error 并点名表/行/列。
USTRUCT(BlueprintType)
struct FCfgPetRow : public FTableRowBase
{
	GENERATED_BODY()

	/** 宝宝种类 id */
	UPROPERTY(BlueprintReadOnly, Category = "Config|Pet")
	int32 id = 0;
	...
	UPROPERTY(BlueprintReadOnly, Category = "Config|Pet")
	TArray<int32> aptitude_min;

	/** 资质上限(万分比),与 aptitude_min 一一对应;出生时在 [min,max] 闭区间随机 */
	UPROPERTY(BlueprintReadOnly, Category = "Config|Pet")
	TArray<int32> aptitude_max;
