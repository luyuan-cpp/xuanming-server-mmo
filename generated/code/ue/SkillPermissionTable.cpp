// ---------------------------------------------------------------------------
//  由 Data Table Exporter 从 SkillPermission.xlsx 生成。**不要手改**。
...
static const TArray<int32> GSkillPermissionEmptyIndices;

/// 把 JSON 里出现过、但 FCfgSkillPermissionRow 里没有对应 UPROPERTY 的键报出来。
///
...
/// SkillPermissionTableCheckNarrowedRow 的事。
static void SkillPermissionTableAuditJsonKeys(const TSharedPtr<FJsonObject>& RowObject, TSet<FString>& CheckedKeys)
{
	const UScriptStruct* RowStruct = FCfgSkillPermissionRow::StaticStruct();
	...
	for (const auto& Pair : RowObject->Values)
	{
		...
		if (RowStruct->FindPropertyByName(FName(*Pair.Key)) == nullptr)
		{
			UE_LOG(LogConfigTable, Warning,
				TEXT("[SkillPermission] skillpermission.json 里的键 '%s' 在 FCfgSkillPermissionRow 中没有对应 UPROPERTY,这一列被静默丢弃。导表器与 UE 产物不同代?"),
				*Pair.Key);
		}
	}
}
...
/// 说明符,见 CfgSkillPermissionRow.h)。收窄本身是刻意的,**但从前是完全静默的**:
...
static void SkillPermissionTableCheckNarrowedValue(const TSharedPtr<FJsonValue>& Value, const TCHAR* ColumnName,
	int32 RowIndex, double MaxValue, const TCHAR* SourceType)
{
	...
	if (Number < 0.0 || Number > MaxValue)
	{
		UE_LOG(LogConfigTable, Error,
			TEXT("[SkillPermission] skillpermission.json data[%d] 的列 '%s'(proto %s)值 %.0f 超出 UE 侧承接类型的范围 [0, %.0f];")
			TEXT("UE 这门把无符号整数收窄成有符号(见 CfgSkillPermissionRow.h),这个值会读成负数或被截断。改用 int64 列或把值调小。"),
			RowIndex, ColumnName, SourceType, Number, MaxValue);
	}
}

/// 取一个键的值做检查;键是数组时逐个元素查。
static void SkillPermissionTableCheckNarrowedField(const TSharedPtr<FJsonObject>& Object, const TCHAR* Key,
	const TCHAR* ColumnName, int32 RowIndex, double MaxValue, const TCHAR* SourceType)
{
	...
	if ((*Field)->Type == EJson::Array)
	{
		...
		{
			SkillPermissionTableCheckNarrowedValue(Element, ColumnName, RowIndex, MaxValue, SourceType);
		}
		return;
	}
	SkillPermissionTableCheckNarrowedValue(*Field, ColumnName, RowIndex, MaxValue, SourceType);
}

/// 一行里全部「由无符号收窄而来」的列的值域检查。
static void SkillPermissionTableCheckNarrowedRow(const TSharedPtr<FJsonObject>& RowObject, int32 RowIndex)
{
	SkillPermissionTableCheckNarrowedField(RowObject, TEXT("id"), TEXT("id"), RowIndex, 2147483647.0, TEXT("uint32"));
	SkillPermissionTableCheckNarrowedField(RowObject, TEXT("skill_type"), TEXT("skill_type[]"), RowIndex, 2147483647.0, TEXT("uint32"));
}

bool USkillPermissionTable::LoadFromJson(const FString& JsonText, FString& OutError)
{
	TSharedPtr<FJsonObject> Root;
	const TSharedRef<TJsonReader<TCHAR>> Reader = TJsonReaderFactory<TCHAR>::Create(JsonText);
	if (!FJsonSerializer::Deserialize(Reader, Root) || !Root.IsValid())
	{
		OutError = FString::Printf(TEXT("[SkillPermission] %s 不是合法 JSON: %s"), TEXT("skillpermission.json"), *Reader->GetErrorMessage());
		return false;
	}
	...
	{
		OutError = TEXT("[SkillPermission] skillpermission.json 顶层缺少数组字段 \"data\"");
		return false;
	}
	...
	for (int32 RowIndex = 0; RowIndex < DataArray.Num(); ++RowIndex)
	{
		...
		{
			OutError = FString::Printf(TEXT("[SkillPermission] data[%d] 不是对象"), RowIndex);
			return false;
		}
		const TSharedPtr<FJsonObject>& RowObject = Element->AsObject();
		SkillPermissionTableAuditJsonKeys(RowObject, CheckedKeys);
		// 值域检查只打日志、不中断装载:一个越界的格子不该让整张表停在旧一代,
		// 但它必须在日志里有名有姓。
		SkillPermissionTableCheckNarrowedRow(RowObject, RowIndex);

		// 必须 value-init:导表器的 JSON **按行省略空单元格**,而 FJsonObjectConverter
		// 对缺席的键是静默跳过、不写。写成 `Row;` 那些列就是未初始化内存 ——
		// 不是 0,不报错,shipping 里一样。
		FCfgSkillPermissionRow Row{};
		// CheckFlags/SkipFlags 都传 0:全部 UPROPERTY 都参与匹配。
		// 匹配靠「属性名 == JSON 键」(TMap<FString> 查找,忽略大小写),所以
		// FCfgSkillPermissionRow 的字段名是 snake_case —— 见 CfgSkillPermissionRow.h 的文件头。
		if (!FJsonObjectConverter::JsonObjectToUStruct<FCfgSkillPermissionRow>(RowObject.ToSharedRef(), &Row, 0, 0))
		{
			OutError = FString::Printf(TEXT("[SkillPermission] data[%d] 转 FCfgSkillPermissionRow 失败"), RowIndex);
			return false;
		}
	...
	UE_LOG(LogConfigTable, Log, TEXT("[SkillPermission] 装载 %d 行"), NewSnapshot->Rows.Num());
	return true;
}

bool USkillPermissionTable::LoadFromDir(const FString& ConfigDir, FString& OutError)
{
	const FString Path = FPaths::Combine(ConfigDir, FileName());
	FString JsonText;
	if (!FFileHelper::LoadFileToString(JsonText, *Path))
	{
		OutError = FString::Printf(TEXT("[SkillPermission] 读不到配置文件: %s"), *Path);
		return false;
	}
	return LoadFromJson(JsonText, OutError);
}

void USkillPermissionTable::BuildIndices(FSnapshot& Snap)
{
	const int32 RowCount = Snap.Rows.Num();
	Snap.IdIndex.Reserve(RowCount);
	for (int32 RowIndex = 0; RowIndex < RowCount; ++RowIndex)
	{
		const FCfgSkillPermissionRow& Row = Snap.Rows[RowIndex];
		// 重复 id 会覆盖前一行。这张表的主键**不是** (cfg_multi),重复即配置错误。
		if (int32* Existing = Snap.IdIndex.Find(Row.id))
		{
			UE_LOG(LogConfigTable, Error,
				TEXT("[SkillPermission] id=%s 重复(行 %d 与行 %d);这张表没有标 (cfg_multi),后一行会覆盖前一行"),
...
const FCfgSkillPermissionRow* USkillPermissionTable::FindById(int32 Id) const
{
	const FCfgSkillPermissionRow* Row = FindByIdSilent(Id);
	if (Row == nullptr)
	{
		UE_LOG(LogConfigTable, Warning, TEXT("[SkillPermission] 查不到 id=%s"), *LexToString(Id));
	}
	return Row;
}

const FCfgSkillPermissionRow* USkillPermissionTable::FindByIdSilent(int32 Id) const
{
	const int32* RowIndex = Snapshot->IdIndex.Find(Id);
	return RowIndex != nullptr ? &Snapshot->Rows[*RowIndex] : nullptr;
}

bool USkillPermissionTable::K2_FindById(int32 Id, FCfgSkillPermissionRow& OutRow) const
{
	if (const FCfgSkillPermissionRow* Row = FindByIdSilent(Id))
	{
		OutRow = *Row;
		return true;
	}
	OutRow = FCfgSkillPermissionRow();
	return false;
}

TArray<const FCfgSkillPermissionRow*> USkillPermissionTable::FindByIds(const TArray<int32>& Ids) const
{
	TArray<const FCfgSkillPermissionRow*> Result;
...
const FCfgSkillPermissionRow* USkillPermissionTable::RandOne() const
{
...
TArray<const FCfgSkillPermissionRow*> USkillPermissionTable::GetRowsBySkillType(int32 Value) const
{
	TArray<const FCfgSkillPermissionRow*> Result;
...
int32 USkillPermissionTable::CountBySkillTypeIndex(int32 Value) const
{
...
TArray<const FCfgSkillPermissionRow*> USkillPermissionTable::Where(TFunctionRef<bool(const FCfgSkillPermissionRow&)> Pred) const
{
	TArray<const FCfgSkillPermissionRow*> Result;
	for (const FCfgSkillPermissionRow& Row : Snapshot->Rows)
	{
		if (Pred(Row))
		{
			Result.Add(&Row);
...
const FCfgSkillPermissionRow* USkillPermissionTable::First(TFunctionRef<bool(const FCfgSkillPermissionRow&)> Pred) const
{
	for (const FCfgSkillPermissionRow& Row : Snapshot->Rows)
	{
		if (Pred(Row))
