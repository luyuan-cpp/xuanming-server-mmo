...
//    generated/tables/guildlevel.json 不是 protojson,是导表器自己 json.dumps 出来的,
...
#include "CfgGuildLevelRow.generated.h"

/// GuildLevel 的一行。字段名与 guildlevel.json 的键逐字符一致(snake_case),
/// 见文件头「字段名口径」。
///
...
/// 这个代价不再是静默的:GuildLevelTable.cpp 在装载路径上对**由无符号收窄而来**的列
/// 做值域检查,越界会打 Error 并点名表/行/列。
USTRUCT(BlueprintType)
struct FCfgGuildLevelRow : public FTableRowBase
{
	GENERATED_BODY()

	/** 帮会等级(从 1 连续) */
	UPROPERTY(BlueprintReadOnly, Category = "Config|GuildLevel")
	int32 id = 0;

	/** 该等级成员上限(>=2,<=100) 上限 100 是帮会推送 MGET 与单帮快照的容量预算前提,改大必须先重做那两处预算 */
	UPROPERTY(BlueprintReadOnly, Category = "Config|GuildLevel")
	int32 max_members = 0;

	/** 该等级长老上限(< max_members) 配表下调导致现有长老超额时不强制降级,只拒绝后续任命;转让时原帮主降为成员 */
	UPROPERTY(BlueprintReadOnly, Category = "Config|GuildLevel")
	int32 max_officers = 0;

	/** 升到下一级所需帮会资金;0 = 满级 只允许最后一级为 0,校验器按"是不是最后一行"与它互为充要条件,防止中间级被误填 0 变成白送 */
	UPROPERTY(BlueprintReadOnly, Category = "Config|GuildLevel")
	int64 upgrade_cost_funds = 0;
};
