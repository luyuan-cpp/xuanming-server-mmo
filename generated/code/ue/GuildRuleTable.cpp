// ---------------------------------------------------------------------------
//  由 Data Table Exporter 从 GuildRule.xlsx 生成。**不要手改**。
...
static const TArray<int32> GGuildRuleEmptyIndices;

/// 把 JSON 里出现过、但 FCfgGuildRuleRow 里没有对应 UPROPERTY 的键报出来。
///
...
/// GuildRuleTableCheckNarrowedRow 的事。
static void GuildRuleTableAuditJsonKeys(const TSharedPtr<FJsonObject>& RowObject, TSet<FString>& CheckedKeys)
{
	const UScriptStruct* RowStruct = FCfgGuildRuleRow::StaticStruct();
	...
	for (const auto& Pair : RowObject->Values)
	{
		...
		if (RowStruct->FindPropertyByName(FName(*Pair.Key)) == nullptr)
		{
			UE_LOG(LogConfigTable, Warning,
				TEXT("[GuildRule] guildrule.json 里的键 '%s' 在 FCfgGuildRuleRow 中没有对应 UPROPERTY,这一列被静默丢弃。导表器与 UE 产物不同代?"),
				*Pair.Key);
		}
	}
}
...
/// 说明符,见 CfgGuildRuleRow.h)。收窄本身是刻意的,**但从前是完全静默的**:
...
static void GuildRuleTableCheckNarrowedValue(const TSharedPtr<FJsonValue>& Value, const TCHAR* ColumnName,
	int32 RowIndex, double MaxValue, const TCHAR* SourceType)
{
	...
	if (Number < 0.0 || Number > MaxValue)
	{
		UE_LOG(LogConfigTable, Error,
			TEXT("[GuildRule] guildrule.json data[%d] 的列 '%s'(proto %s)值 %.0f 超出 UE 侧承接类型的范围 [0, %.0f];")
			TEXT("UE 这门把无符号整数收窄成有符号(见 CfgGuildRuleRow.h),这个值会读成负数或被截断。改用 int64 列或把值调小。"),
			RowIndex, ColumnName, SourceType, Number, MaxValue);
	}
}

/// 取一个键的值做检查;键是数组时逐个元素查。
static void GuildRuleTableCheckNarrowedField(const TSharedPtr<FJsonObject>& Object, const TCHAR* Key,
	const TCHAR* ColumnName, int32 RowIndex, double MaxValue, const TCHAR* SourceType)
{
	...
	if ((*Field)->Type == EJson::Array)
	{
		...
		{
			GuildRuleTableCheckNarrowedValue(Element, ColumnName, RowIndex, MaxValue, SourceType);
		}
		return;
	}
	GuildRuleTableCheckNarrowedValue(*Field, ColumnName, RowIndex, MaxValue, SourceType);
}

/// 一行里全部「由无符号收窄而来」的列的值域检查。
static void GuildRuleTableCheckNarrowedRow(const TSharedPtr<FJsonObject>& RowObject, int32 RowIndex)
{
	GuildRuleTableCheckNarrowedField(RowObject, TEXT("id"), TEXT("id"), RowIndex, 2147483647.0, TEXT("uint32"));
	GuildRuleTableCheckNarrowedField(RowObject, TEXT("application_expire_hours"), TEXT("application_expire_hours"), RowIndex, 2147483647.0, TEXT("uint32"));
	GuildRuleTableCheckNarrowedField(RowObject, TEXT("max_pending_applications_per_player"), TEXT("max_pending_applications_per_player"), RowIndex, 2147483647.0, TEXT("uint32"));
	GuildRuleTableCheckNarrowedField(RowObject, TEXT("max_pending_applications_per_guild"), TEXT("max_pending_applications_per_guild"), RowIndex, 2147483647.0, TEXT("uint32"));
...
bool UGuildRuleTable::LoadFromJson(const FString& JsonText, FString& OutError)
{
	...
	{
		OutError = FString::Printf(TEXT("[GuildRule] %s 不是合法 JSON: %s"), TEXT("guildrule.json"), *Reader->GetErrorMessage());
		return false;
	}
	...
	{
		OutError = TEXT("[GuildRule] guildrule.json 顶层缺少数组字段 \"data\"");
		return false;
	}
	...
	for (int32 RowIndex = 0; RowIndex < DataArray.Num(); ++RowIndex)
	{
		...
		{
			OutError = FString::Printf(TEXT("[GuildRule] data[%d] 不是对象"), RowIndex);
			return false;
		}
		const TSharedPtr<FJsonObject>& RowObject = Element->AsObject();
		GuildRuleTableAuditJsonKeys(RowObject, CheckedKeys);
		// 值域检查只打日志、不中断装载:一个越界的格子不该让整张表停在旧一代,
		// 但它必须在日志里有名有姓。
		GuildRuleTableCheckNarrowedRow(RowObject, RowIndex);

		// 必须 value-init:导表器的 JSON **按行省略空单元格**,而 FJsonObjectConverter
		// 对缺席的键是静默跳过、不写。写成 `Row;` 那些列就是未初始化内存 ——
		// 不是 0,不报错,shipping 里一样。
		FCfgGuildRuleRow Row{};
		// CheckFlags/SkipFlags 都传 0:全部 UPROPERTY 都参与匹配。
		// 匹配靠「属性名 == JSON 键」(TMap<FString> 查找,忽略大小写),所以
		// FCfgGuildRuleRow 的字段名是 snake_case —— 见 CfgGuildRuleRow.h 的文件头。
		if (!FJsonObjectConverter::JsonObjectToUStruct<FCfgGuildRuleRow>(RowObject.ToSharedRef(), &Row, 0, 0))
		{
			OutError = FString::Printf(TEXT("[GuildRule] data[%d] 转 FCfgGuildRuleRow 失败"), RowIndex);
			return false;
		}
	...
	UE_LOG(LogConfigTable, Log, TEXT("[GuildRule] 装载 %d 行"), NewSnapshot->Rows.Num());
	return true;
}

bool UGuildRuleTable::LoadFromDir(const FString& ConfigDir, FString& OutError)
{
	const FString Path = FPaths::Combine(ConfigDir, FileName());
	FString JsonText;
	if (!FFileHelper::LoadFileToString(JsonText, *Path))
	{
		OutError = FString::Printf(TEXT("[GuildRule] 读不到配置文件: %s"), *Path);
		return false;
	}
	return LoadFromJson(JsonText, OutError);
}

void UGuildRuleTable::BuildIndices(FSnapshot& Snap)
{
	const int32 RowCount = Snap.Rows.Num();
	Snap.IdIndex.Reserve(RowCount);
	for (int32 RowIndex = 0; RowIndex < RowCount; ++RowIndex)
	{
		const FCfgGuildRuleRow& Row = Snap.Rows[RowIndex];
		// 重复 id 会覆盖前一行。这张表的主键**不是** (cfg_multi),重复即配置错误。
		if (int32* Existing = Snap.IdIndex.Find(Row.id))
		{
			UE_LOG(LogConfigTable, Error,
				TEXT("[GuildRule] id=%s 重复(行 %d 与行 %d);这张表没有标 (cfg_multi),后一行会覆盖前一行"),
...
const FCfgGuildRuleRow* UGuildRuleTable::FindById(int32 Id) const
{
	const FCfgGuildRuleRow* Row = FindByIdSilent(Id);
	if (Row == nullptr)
	{
		UE_LOG(LogConfigTable, Warning, TEXT("[GuildRule] 查不到 id=%s"), *LexToString(Id));
	}
	return Row;
}

const FCfgGuildRuleRow* UGuildRuleTable::FindByIdSilent(int32 Id) const
{
	const int32* RowIndex = Snapshot->IdIndex.Find(Id);
	return RowIndex != nullptr ? &Snapshot->Rows[*RowIndex] : nullptr;
}

bool UGuildRuleTable::K2_FindById(int32 Id, FCfgGuildRuleRow& OutRow) const
{
	if (const FCfgGuildRuleRow* Row = FindByIdSilent(Id))
	{
		OutRow = *Row;
		return true;
	}
	OutRow = FCfgGuildRuleRow();
	return false;
}

TArray<const FCfgGuildRuleRow*> UGuildRuleTable::FindByIds(const TArray<int32>& Ids) const
{
	TArray<const FCfgGuildRuleRow*> Result;
...
const FCfgGuildRuleRow* UGuildRuleTable::RandOne() const
{
...
TArray<const FCfgGuildRuleRow*> UGuildRuleTable::Where(TFunctionRef<bool(const FCfgGuildRuleRow&)> Pred) const
{
	TArray<const FCfgGuildRuleRow*> Result;
	for (const FCfgGuildRuleRow& Row : Snapshot->Rows)
	{
		if (Pred(Row))
		{
			Result.Add(&Row);
...
const FCfgGuildRuleRow* UGuildRuleTable::First(TFunctionRef<bool(const FCfgGuildRuleRow&)> Pred) const
{
	for (const FCfgGuildRuleRow& Row : Snapshot->Rows)
	{
		if (Pred(Row))
