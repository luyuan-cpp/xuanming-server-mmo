// ---------------------------------------------------------------------------
//  由 Data Table Exporter 从 Class.xlsx 生成。**不要手改**。
...
static const TArray<int32> GClassEmptyIndices;

/// 把 JSON 里出现过、但 FCfgClassRow 里没有对应 UPROPERTY 的键报出来。
///
...
/// ClassTableCheckNarrowedRow 的事。
static void ClassTableAuditJsonKeys(const TSharedPtr<FJsonObject>& RowObject, TSet<FString>& CheckedKeys)
{
	const UScriptStruct* RowStruct = FCfgClassRow::StaticStruct();
	...
	for (const auto& Pair : RowObject->Values)
	{
		...
		if (RowStruct->FindPropertyByName(FName(*Pair.Key)) == nullptr)
		{
			UE_LOG(LogConfigTable, Warning,
				TEXT("[Class] class.json 里的键 '%s' 在 FCfgClassRow 中没有对应 UPROPERTY,这一列被静默丢弃。导表器与 UE 产物不同代?"),
				*Pair.Key);
		}
	}
}
...
/// 说明符,见 CfgClassRow.h)。收窄本身是刻意的,**但从前是完全静默的**:
...
static void ClassTableCheckNarrowedValue(const TSharedPtr<FJsonValue>& Value, const TCHAR* ColumnName,
	int32 RowIndex, double MaxValue, const TCHAR* SourceType)
{
	...
	if (Number < 0.0 || Number > MaxValue)
	{
		UE_LOG(LogConfigTable, Error,
			TEXT("[Class] class.json data[%d] 的列 '%s'(proto %s)值 %.0f 超出 UE 侧承接类型的范围 [0, %.0f];")
			TEXT("UE 这门把无符号整数收窄成有符号(见 CfgClassRow.h),这个值会读成负数或被截断。改用 int64 列或把值调小。"),
			RowIndex, ColumnName, SourceType, Number, MaxValue);
	}
}

/// 取一个键的值做检查;键是数组时逐个元素查。
static void ClassTableCheckNarrowedField(const TSharedPtr<FJsonObject>& Object, const TCHAR* Key,
	const TCHAR* ColumnName, int32 RowIndex, double MaxValue, const TCHAR* SourceType)
{
	...
	if ((*Field)->Type == EJson::Array)
	{
		...
		{
			ClassTableCheckNarrowedValue(Element, ColumnName, RowIndex, MaxValue, SourceType);
		}
		return;
	}
	ClassTableCheckNarrowedValue(*Field, ColumnName, RowIndex, MaxValue, SourceType);
}
...
bool UClassTable::LoadFromJson(const FString& JsonText, FString& OutError)
{
	TSharedPtr<FJsonObject> Root;
	const TSharedRef<TJsonReader<TCHAR>> Reader = TJsonReaderFactory<TCHAR>::Create(JsonText);
	if (!FJsonSerializer::Deserialize(Reader, Root) || !Root.IsValid())
	{
		OutError = FString::Printf(TEXT("[Class] %s 不是合法 JSON: %s"), TEXT("class.json"), *Reader->GetErrorMessage());
		return false;
	}
	...
	{
		OutError = TEXT("[Class] class.json 顶层缺少数组字段 \"data\"");
		return false;
	}
	...
	for (int32 RowIndex = 0; RowIndex < DataArray.Num(); ++RowIndex)
	{
		...
		{
			OutError = FString::Printf(TEXT("[Class] data[%d] 不是对象"), RowIndex);
			return false;
		}
		const TSharedPtr<FJsonObject>& RowObject = Element->AsObject();
		ClassTableAuditJsonKeys(RowObject, CheckedKeys);
		// 值域检查只打日志、不中断装载:一个越界的格子不该让整张表停在旧一代,
		// 但它必须在日志里有名有姓。
		ClassTableCheckNarrowedRow(RowObject, RowIndex);
		...
		// FCfgClassRow 的字段名是 snake_case —— 见 CfgClassRow.h 的文件头。
		if (!FJsonObjectConverter::JsonObjectToUStruct<FCfgClassRow>(RowObject.ToSharedRef(), &Row, 0, 0))
		{
			OutError = FString::Printf(TEXT("[Class] data[%d] 转 FCfgClassRow 失败"), RowIndex);
			return false;
		}
	...
	UE_LOG(LogConfigTable, Log, TEXT("[Class] 装载 %d 行"), NewSnapshot->Rows.Num());
	return true;
}

bool UClassTable::LoadFromDir(const FString& ConfigDir, FString& OutError)
{
	const FString Path = FPaths::Combine(ConfigDir, FileName());
	FString JsonText;
	if (!FFileHelper::LoadFileToString(JsonText, *Path))
	{
		OutError = FString::Printf(TEXT("[Class] 读不到配置文件: %s"), *Path);
		return false;
	}
	return LoadFromJson(JsonText, OutError);
}

void UClassTable::BuildIndices(FSnapshot& Snap)
{
	const int32 RowCount = Snap.Rows.Num();
	Snap.IdIndex.Reserve(RowCount);
	for (int32 RowIndex = 0; RowIndex < RowCount; ++RowIndex)
	{
		const FCfgClassRow& Row = Snap.Rows[RowIndex];
		// 重复 id 会覆盖前一行。这张表的主键**不是** (cfg_multi),重复即配置错误。
		if (int32* Existing = Snap.IdIndex.Find(Row.id))
		{
			UE_LOG(LogConfigTable, Error,
				TEXT("[Class] id=%s 重复(行 %d 与行 %d);这张表没有标 (cfg_multi),后一行会覆盖前一行"),
...
const FCfgClassRow* UClassTable::FindById(int32 Id) const
{
	const FCfgClassRow* Row = FindByIdSilent(Id);
	if (Row == nullptr)
	{
		UE_LOG(LogConfigTable, Warning, TEXT("[Class] 查不到 id=%s"), *LexToString(Id));
	}
	return Row;
}

const FCfgClassRow* UClassTable::FindByIdSilent(int32 Id) const
{
	const int32* RowIndex = Snapshot->IdIndex.Find(Id);
	return RowIndex != nullptr ? &Snapshot->Rows[*RowIndex] : nullptr;
}

bool UClassTable::K2_FindById(int32 Id, FCfgClassRow& OutRow) const
{
	if (const FCfgClassRow* Row = FindByIdSilent(Id))
	{
		OutRow = *Row;
		return true;
	}
...
TArray<const FCfgClassRow*> UClassTable::FindByIds(const TArray<int32>& Ids) const
{
	TArray<const FCfgClassRow*> Result;
...
const FCfgClassRow* UClassTable::RandOne() const
{
...
TArray<const FCfgClassRow*> UClassTable::GetRowsBySkill(int32 Value) const
{
	TArray<const FCfgClassRow*> Result;
...
int32 UClassTable::CountBySkillIndex(int32 Value) const
{
...
TArray<const FCfgClassRow*> UClassTable::Where(TFunctionRef<bool(const FCfgClassRow&)> Pred) const
{
	TArray<const FCfgClassRow*> Result;
	for (const FCfgClassRow& Row : Snapshot->Rows)
	{
		if (Pred(Row))
		{
			Result.Add(&Row);
...
const FCfgClassRow* UClassTable::First(TFunctionRef<bool(const FCfgClassRow&)> Pred) const
{
	for (const FCfgClassRow& Row : Snapshot->Rows)
	{
		if (Pred(Row))
