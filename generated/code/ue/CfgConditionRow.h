...
//    generated/tables/condition.json 不是 protojson,是导表器自己 json.dumps 出来的,
...
#include "CfgConditionRow.generated.h"

/// Condition 的一行。字段名与 condition.json 的键逐字符一致(snake_case),
/// 见文件头「字段名口径」。
///
...
/// 这个代价不再是静默的:ConditionTable.cpp 在装载路径上对**由无符号收窄而来**的列
/// 做值域检查,越界会打 Error 并点名表/行/列。
USTRUCT(BlueprintType)
struct FCfgConditionRow : public FTableRowBase
{
	GENERATED_BODY()

	UPROPERTY(BlueprintReadOnly, Category = "Config|Condition")
	int32 id = 0;

	/** 1杀怪2对话npc3完成相应条件4使用类型物品5交互6升级7不是表里的条件8完成相应任务 */
	UPROPERTY(BlueprintReadOnly, Category = "Config|Condition")
	int32 condition_category = 0;

	UPROPERTY(BlueprintReadOnly, Category = "Config|Condition")
	TArray<int32> condition1;

	UPROPERTY(BlueprintReadOnly, Category = "Config|Condition")
	TArray<int32> condition2;

	UPROPERTY(BlueprintReadOnly, Category = "Config|Condition")
	TArray<int32> condition3;

	UPROPERTY(BlueprintReadOnly, Category = "Config|Condition")
	TArray<int32> condition4;

	/** 时间约束（如 1 小时内完成） */
	UPROPERTY(BlueprintReadOnly, Category = "Config|Condition")
	int64 valid_duration = 0;

	/** 0:累计 (如累计充值人民币1万元) 1:拥有 (如拥有一万砖石)并用枚举描述 0 = cumulative, 1 = owned */
	UPROPERTY(BlueprintReadOnly, Category = "Config|Condition")
	int32 quantity_type = 0;
