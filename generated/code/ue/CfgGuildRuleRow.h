...
//    generated/tables/guildrule.json 不是 protojson,是导表器自己 json.dumps 出来的,
...
#include "CfgGuildRuleRow.generated.h"

/// GuildRule 的一行。字段名与 guildrule.json 的键逐字符一致(snake_case),
/// 见文件头「字段名口径」。
///
...
/// 这个代价不再是静默的:GuildRuleTable.cpp 在装载路径上对**由无符号收窄而来**的列
/// 做值域检查,越界会打 Error 并点名表/行/列。
USTRUCT(BlueprintType)
struct FCfgGuildRuleRow : public FTableRowBase
{
	GENERATED_BODY()
	...
	/** 入帮申请有效期(小时),1-720 过期的申请在下一次写事务里顺手删掉,不占用下面两个上限;调小会让存量申请立刻失效 */
	UPROPERTY(BlueprintReadOnly, Category = "Config|GuildRule")
	int32 application_expire_hours = 0;

	/** 每个玩家同时待审的申请上限,1-10 在申请人的 guild_player_state 行锁下计数,并发申请被串行化,所以这是硬上限不是估算值 */
	UPROPERTY(BlueprintReadOnly, Category = "Config|GuildRule")
	int32 max_pending_applications_per_player = 0;

	/** 每个帮会同时待审的申请上限,1-500 防止外人灌满审批列表:超过即拒新申请,已入帮成员不受影响 */
	UPROPERTY(BlueprintReadOnly, Category = "Config|GuildRule")
	int32 max_pending_applications_per_guild = 0;

	/** 资产操作截止秒数(B5 读取) 捐献指令超过它还没结算就改发中止,避免玩家货币被扣住却永远拿不到帮贡 */
	UPROPERTY(BlueprintReadOnly, Category = "Config|GuildRule")
	int32 asset_op_deadline_seconds = 0;

	/** 资产操作重投基准毫秒(B5 读取) 后台重投的退避基数(会叠加抖动),不是单次调用的超时时间 */
	UPROPERTY(BlueprintReadOnly, Category = "Config|GuildRule")
	int32 asset_op_retry_base_ms = 0;
