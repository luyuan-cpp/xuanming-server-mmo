...
//    generated/tables/guilddonate.json 不是 protojson,是导表器自己 json.dumps 出来的,
...
#include "CfgGuildDonateRow.generated.h"

/// GuildDonate 的一行。字段名与 guilddonate.json 的键逐字符一致(snake_case),
/// 见文件头「字段名口径」。
///
...
/// 这个代价不再是静默的:GuildDonateTable.cpp 在装载路径上对**由无符号收窄而来**的列
/// 做值域检查,越界会打 Error 并点名表/行/列。
USTRUCT(BlueprintType)
struct FCfgGuildDonateRow : public FTableRowBase
{
	GENERATED_BODY()

	/** 捐献选项 id */
	UPROPERTY(BlueprintReadOnly, Category = "Config|GuildDonate")
	int32 id = 0;
	...
	/** 货币类型:0 银两(kCurrencyGold)/ 1 灵石(kCurrencyDiamond) 绑定灵石不能捐;其它值启动拒绝 */
	UPROPERTY(BlueprintReadOnly, Category = "Config|GuildDonate")
	int32 currency_type = 0;

	/** 每次扣除的货币数量(1..int64 上限,scene Debit 的要求) */
	UPROPERTY(BlueprintReadOnly, Category = "Config|GuildDonate")
	int64 cost_amount = 0;

	/** 每次获得的个人帮贡(累计与可用同加) contribution_gain 与 funds_gain 至少一个 > 0 */
	UPROPERTY(BlueprintReadOnly, Category = "Config|GuildDonate")
	int64 contribution_gain = 0;
