...
//    generated/tables/petrule.json 不是 protojson,是导表器自己 json.dumps 出来的,
...
#include "CfgPetRuleRow.generated.h"

/// PetRule 的一行。字段名与 petrule.json 的键逐字符一致(snake_case),
/// 见文件头「字段名口径」。
///
...
/// 这个代价不再是静默的:PetRuleTable.cpp 在装载路径上对**由无符号收窄而来**的列
/// 做值域检查,越界会打 Error 并点名表/行/列。
USTRUCT(BlueprintType)
struct FCfgPetRuleRow : public FTableRowBase
{
	GENERATED_BODY()

	/** 规则行 id(固定 1) */
	UPROPERTY(BlueprintReadOnly, Category = "Config|PetRule")
	int32 id = 0;
	...
	/** 宝宝名最大长度(UTF-8 码点数) */
	UPROPERTY(BlueprintReadOnly, Category = "Config|PetRule")
	int32 name_max_len = 0;

	/** 改名金币(0 = 免费) */
	UPROPERTY(BlueprintReadOnly, Category = "Config|PetRule")
	int64 rename_cost_gold = 0;

	/** 召唤/收回冷却秒(0 = 无冷却) */
	UPROPERTY(BlueprintReadOnly, Category = "Config|PetRule")
	int32 summon_cooldown_seconds = 0;
