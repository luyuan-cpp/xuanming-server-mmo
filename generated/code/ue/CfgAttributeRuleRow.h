...
//    generated/tables/attributerule.json 不是 protojson,是导表器自己 json.dumps 出来的,
...
#include "CfgAttributeRuleRow.generated.h"

/// AttributeRule 的一行。字段名与 attributerule.json 的键逐字符一致(snake_case),
/// 见文件头「字段名口径」。
///
...
/// 这个代价不再是静默的:AttributeRuleTable.cpp 在装载路径上对**由无符号收窄而来**的列
/// 做值域检查,越界会打 Error 并点名表/行/列。
USTRUCT(BlueprintType)
struct FCfgAttributeRuleRow : public FTableRowBase
{
	GENERATED_BODY()
	...
	/** 免费方案数(含默认方案) */
	UPROPERTY(BlueprintReadOnly, Category = "Config|AttributeRule")
	int32 free_scheme_count = 0;

	/** 开启新方案金币(超出免费数后) */
	UPROPERTY(BlueprintReadOnly, Category = "Config|AttributeRule")
	int64 create_scheme_cost_gold = 0;

	/** 切换方案冷却(秒) */
	UPROPERTY(BlueprintReadOnly, Category = "Config|AttributeRule")
	int32 switch_cooldown_seconds = 0;
