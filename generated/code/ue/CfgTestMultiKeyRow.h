...
//    generated/tables/testmultikey.json 不是 protojson,是导表器自己 json.dumps 出来的,
...
#include "CfgTestMultiKeyRow.generated.h"

/// TestMultiKey.testobj1 的子消息行。
USTRUCT(BlueprintType)
struct FCfgTestMultiKeyTestobj1
{
	GENERATED_BODY()

	/** 等级 */
	UPROPERTY(BlueprintReadOnly, Category = "Config|TestMultiKey")
	int32 testobj1_key = 0;

	UPROPERTY(BlueprintReadOnly, Category = "Config|TestMultiKey")
	int32 testobj1_value = 0;
};

/// TestMultiKey 的一行。字段名与 testmultikey.json 的键逐字符一致(snake_case),
/// 见文件头「字段名口径」。
///
...
/// 这个代价不再是静默的:TestMultiKeyTable.cpp 在装载路径上对**由无符号收窄而来**的列
/// 做值域检查,越界会打 Error 并点名表/行/列。
USTRUCT(BlueprintType)
struct FCfgTestMultiKeyRow : public FTableRowBase
{
	GENERATED_BODY()
	...
	UPROPERTY(BlueprintReadOnly, Category = "Config|TestMultiKey")
	FString m_string_key;

	UPROPERTY(BlueprintReadOnly, Category = "Config|TestMultiKey")
	int32 m_uint32_key = 0;
	...
	/** GFK to Test table */
	UPROPERTY(BlueprintReadOnly, Category = "Config|TestMultiKey")
	TArray<int32> test_refs;

	UPROPERTY(BlueprintReadOnly, Category = "Config|TestMultiKey")
	TMap<FString, FString> tag1;

	UPROPERTY(BlueprintReadOnly, Category = "Config|TestMultiKey")
	TMap<FString, FString> immune_tag1;

	UPROPERTY(BlueprintReadOnly, Category = "Config|TestMultiKey")
	TArray<FCfgTestMultiKeyTestobj1> testobj1;
};
