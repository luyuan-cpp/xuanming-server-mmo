...
static const TArray<int32> GWorldEmptyIndices;

/// 把 JSON 里出现过、但 FCfgWorldRow 里没有对应 UPROPERTY 的键报出来。
///
...
/// WorldTableCheckNarrowedRow 的事。
static void WorldTableAuditJsonKeys(const TSharedPtr<FJsonObject>& RowObject, TSet<FString>& CheckedKeys)
{
	const UScriptStruct* RowStruct = FCfgWorldRow::StaticStruct();
	...
	for (const auto& Pair : RowObject->Values)
	{
		...
		if (RowStruct->FindPropertyByName(FName(*Pair.Key)) == nullptr)
		{
			UE_LOG(LogConfigTable, Warning,
				TEXT("[World] world.json 里的键 '%s' 在 FCfgWorldRow 中没有对应 UPROPERTY,这一列被静默丢弃。导表器与 UE 产物不同代?"),
				*Pair.Key);
		}
	}
}
...
/// 说明符,见 CfgWorldRow.h)。收窄本身是刻意的,**但从前是完全静默的**:
...
static void WorldTableCheckNarrowedValue(const TSharedPtr<FJsonValue>& Value, const TCHAR* ColumnName,
	int32 RowIndex, double MaxValue, const TCHAR* SourceType)
{
	...
	if (Number < 0.0 || Number > MaxValue)
	{
		UE_LOG(LogConfigTable, Error,
			TEXT("[World] world.json data[%d] 的列 '%s'(proto %s)值 %.0f 超出 UE 侧承接类型的范围 [0, %.0f];")
			TEXT("UE 这门把无符号整数收窄成有符号(见 CfgWorldRow.h),这个值会读成负数或被截断。改用 int64 列或把值调小。"),
			RowIndex, ColumnName, SourceType, Number, MaxValue);
	}
}

/// 取一个键的值做检查;键是数组时逐个元素查。
static void WorldTableCheckNarrowedField(const TSharedPtr<FJsonObject>& Object, const TCHAR* Key,
	const TCHAR* ColumnName, int32 RowIndex, double MaxValue, const TCHAR* SourceType)
{
	...
	if ((*Field)->Type == EJson::Array)
	{
		...
		{
			WorldTableCheckNarrowedValue(Element, ColumnName, RowIndex, MaxValue, SourceType);
		}
		return;
	}
	WorldTableCheckNarrowedValue(*Field, ColumnName, RowIndex, MaxValue, SourceType);
}

/// 一行里全部「由无符号收窄而来」的列的值域检查。
static void WorldTableCheckNarrowedRow(const TSharedPtr<FJsonObject>& RowObject, int32 RowIndex)
{
	WorldTableCheckNarrowedField(RowObject, TEXT("id"), TEXT("id"), RowIndex, 2147483647.0, TEXT("uint32"));
	WorldTableCheckNarrowedField(RowObject, TEXT("scene_id"), TEXT("scene_id"), RowIndex, 2147483647.0, TEXT("uint32"));
}

bool UWorldTable::LoadFromJson(const FString& JsonText, FString& OutError)
{
	TSharedPtr<FJsonObject> Root;
	const TSharedRef<TJsonReader<TCHAR>> Reader = TJsonReaderFactory<TCHAR>::Create(JsonText);
	if (!FJsonSerializer::Deserialize(Reader, Root) || !Root.IsValid())
	{
		OutError = FString::Printf(TEXT("[World] %s 不是合法 JSON: %s"), TEXT("world.json"), *Reader->GetErrorMessage());
		return false;
	}
	...
	{
		OutError = TEXT("[World] world.json 顶层缺少数组字段 \"data\"");
		return false;
	}
	...
	for (int32 RowIndex = 0; RowIndex < DataArray.Num(); ++RowIndex)
	{
		...
		{
			OutError = FString::Printf(TEXT("[World] data[%d] 不是对象"), RowIndex);
			return false;
		}
		const TSharedPtr<FJsonObject>& RowObject = Element->AsObject();
		WorldTableAuditJsonKeys(RowObject, CheckedKeys);
		// 值域检查只打日志、不中断装载:一个越界的格子不该让整张表停在旧一代,
		// 但它必须在日志里有名有姓。
		WorldTableCheckNarrowedRow(RowObject, RowIndex);
		...
		// FCfgWorldRow 的字段名是 snake_case —— 见 CfgWorldRow.h 的文件头。
		if (!FJsonObjectConverter::JsonObjectToUStruct<FCfgWorldRow>(RowObject.ToSharedRef(), &Row, 0, 0))
		{
			OutError = FString::Printf(TEXT("[World] data[%d] 转 FCfgWorldRow 失败"), RowIndex);
			return false;
		}
	...
	UE_LOG(LogConfigTable, Log, TEXT("[World] 装载 %d 行"), NewSnapshot->Rows.Num());
	return true;
}

bool UWorldTable::LoadFromDir(const FString& ConfigDir, FString& OutError)
{
	const FString Path = FPaths::Combine(ConfigDir, FileName());
	FString JsonText;
	if (!FFileHelper::LoadFileToString(JsonText, *Path))
	{
		OutError = FString::Printf(TEXT("[World] 读不到配置文件: %s"), *Path);
		return false;
	}
	return LoadFromJson(JsonText, OutError);
}

void UWorldTable::BuildIndices(FSnapshot& Snap)
{
	const int32 RowCount = Snap.Rows.Num();
	Snap.IdIndex.Reserve(RowCount);
	for (int32 RowIndex = 0; RowIndex < RowCount; ++RowIndex)
	{
		const FCfgWorldRow& Row = Snap.Rows[RowIndex];
		// 重复 id 会覆盖前一行。这张表的主键**不是** (cfg_multi),重复即配置错误。
		if (int32* Existing = Snap.IdIndex.Find(Row.id))
		{
			UE_LOG(LogConfigTable, Error,
				TEXT("[World] id=%s 重复(行 %d 与行 %d);这张表没有标 (cfg_multi),后一行会覆盖前一行"),
...
const FCfgWorldRow* UWorldTable::FindById(int32 Id) const
{
	const FCfgWorldRow* Row = FindByIdSilent(Id);
	if (Row == nullptr)
	{
		UE_LOG(LogConfigTable, Warning, TEXT("[World] 查不到 id=%s"), *LexToString(Id));
	}
	return Row;
}

const FCfgWorldRow* UWorldTable::FindByIdSilent(int32 Id) const
{
	const int32* RowIndex = Snapshot->IdIndex.Find(Id);
	return RowIndex != nullptr ? &Snapshot->Rows[*RowIndex] : nullptr;
}

bool UWorldTable::K2_FindById(int32 Id, FCfgWorldRow& OutRow) const
{
	if (const FCfgWorldRow* Row = FindByIdSilent(Id))
	{
		OutRow = *Row;
		return true;
	}
...
TArray<const FCfgWorldRow*> UWorldTable::FindByIds(const TArray<int32>& Ids) const
{
	TArray<const FCfgWorldRow*> Result;
...
const FCfgWorldRow* UWorldTable::RandOne() const
{
...
TArray<const FCfgWorldRow*> UWorldTable::GetBySceneId(int32 Key) const
{
	TArray<const FCfgWorldRow*> Result;
...
const TArray<int32>& UWorldTable::GetSceneIdIndices(int32 Key) const
{
	const TArray<int32>* Found = Snapshot->SceneIdIndex.Find(Key);
	return Found != nullptr ? *Found : GWorldEmptyIndices;
}

int32 UWorldTable::CountBySceneIdIndex(int32 Key) const
{
	return GetSceneIdIndices(Key).Num();
}

TArray<const FCfgWorldRow*> UWorldTable::Where(TFunctionRef<bool(const FCfgWorldRow&)> Pred) const
{
	TArray<const FCfgWorldRow*> Result;
	for (const FCfgWorldRow& Row : Snapshot->Rows)
	{
		if (Pred(Row))
		{
			Result.Add(&Row);
...
const FCfgWorldRow* UWorldTable::First(TFunctionRef<bool(const FCfgWorldRow&)> Pred) const
{
	for (const FCfgWorldRow& Row : Snapshot->Rows)
	{
		if (Pred(Row))
