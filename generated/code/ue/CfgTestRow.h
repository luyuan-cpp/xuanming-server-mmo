...
//    generated/tables/test.json 不是 protojson,是导表器自己 json.dumps 出来的,
...
#include "CfgTestRow.generated.h"

/// Test.testobj 的子消息行。
USTRUCT(BlueprintType)
struct FCfgTestTestobj
{
	GENERATED_BODY()

	/** 等级 */
	UPROPERTY(BlueprintReadOnly, Category = "Config|Test")
	int32 testobj_key = 0;

	UPROPERTY(BlueprintReadOnly, Category = "Config|Test")
	int32 testobj_value = 0;
};

/// Test 的一行。字段名与 test.json 的键逐字符一致(snake_case),
/// 见文件头「字段名口径」。
///
...
/// 这个代价不再是静默的:TestTable.cpp 在装载路径上对**由无符号收窄而来**的列
/// 做值域检查,越界会打 Error 并点名表/行/列。
USTRUCT(BlueprintType)
struct FCfgTestRow : public FTableRowBase
{
	GENERATED_BODY()
	...
	UPROPERTY(BlueprintReadOnly, Category = "Config|Test")
	TMap<FString, FString> tag;

	UPROPERTY(BlueprintReadOnly, Category = "Config|Test")
	TMap<FString, FString> immune_tag;

	UPROPERTY(BlueprintReadOnly, Category = "Config|Test")
	TArray<FCfgTestTestobj> testobj;
};
