...
// ---------------------------------------------------------------------------
//
//  模块依赖:本头要 Core + CoreUObject;配套的 MonsterTable.cpp 还要 Json +
...
struct FCfgMonsterSnapshot
{
	/// 全部行,按 monster.json 里的出现顺序。
	TArray<FCfgMonsterRow> Rows;
	/// id -> 行下标。
 ...
 * ---- 生命周期契约(热更)----
 *
 * Load*() 会**整批**建好一个新的 FCfgMonsterSnapshot 再原子替换成员指针;旧快照
 * 在最后一个持有者放手时析构。所以下面所有返回 `const FCfgMonsterRow*` /
 * `const TArray<...>&` / `const FCfgMonsterSnapshot&` 的接口,
 * **返回值只在下一次 Load 之前有效**:
 *
 *     ✅ 存 id,用的时候现查
 *     ❌ 把 const FCfgMonsterRow* 存进成员、容器、Lambda 捕获、Latent 节点
 *     ❌ 把**行下标**(int32)存起来 —— 下标只在取它的那一代快照里有意义。
 *        它可平凡拷贝,不会像野指针那样崩;换代之后是**静默读到另一行**,更难查。
 *
 * UE 的 GC **救不了你**:FCfgMonsterRow 是 USTRUCT 不是 UObject,它躺在一个
 ...
 *
 *     TSharedPtr<const FCfgMonsterSnapshot> Pinned = Table->PinSnapshot();
 *     const FCfgMonsterRow* Row = Pinned->Rows.IsValidIndex(I) ? &Pinned->Rows[I] : nullptr;
 *
 * ---- 线程契约 ----
 *
...
class MMORPGCONFIG_API UMonsterTable : public UObject
{
	GENERATED_BODY()

public:
	using FSnapshot = FCfgMonsterSnapshot;

	/// 这张表在 generated/tables 下的文件名(全小写,与 .pb 同口径)。
	static const TCHAR* FileName() { return TEXT("monster.json"); }
	...
	const TArray<FCfgMonsterRow>& GetRows() const { return Snapshot->Rows; }

	/// 按行下标取行;越界返回 nullptr。
	const FCfgMonsterRow* RowAt(int32 RowIndex) const
	{
	...
	const FCfgMonsterRow* FindById(int32 Id) const;

	/// 命中不到返回 nullptr,不打日志(「查不到是正常分支」的场景用这个)。
	const FCfgMonsterRow* FindByIdSilent(int32 Id) const;

	UFUNCTION(BlueprintCallable, Category = "Config|Monster", DisplayName = "Find By Id")
	bool K2_FindById(int32 Id, FCfgMonsterRow& OutRow) const;

	UFUNCTION(BlueprintPure, Category = "Config|Monster")
	...
	TArray<const FCfgMonsterRow*> FindByIds(const TArray<int32>& Ids) const;

	/** 随机取一行。表为空返回 nullptr。用 FMath::RandRange,不引 <random>。 */
	const FCfgMonsterRow* RandOne() const;
	...
	TArray<const FCfgMonsterRow*> Where(TFunctionRef<bool(const FCfgMonsterRow&)> Pred) const;
	const FCfgMonsterRow* First(TFunctionRef<bool(const FCfgMonsterRow&)> Pred) const;
