"""AttributeRule 配置表管理器（自动生成，请勿手改）。

DO NOT EDIT — regenerate from Excel via Data Table Exporter.

热更契约
========
:meth:`AttributeRuleTableManager.load` **整批**建好一个新的不可变快照，再一次性
...
import attributerule_table_pb2 as _pb

#: 这张表的行类型。所有查询返回的都是它，或者它的元组。
Row = _pb.AttributeRuleTable
...
class Snapshot:
    ...
    container: _pb.AttributeRuleTableData
    #: 表内顺序的全部行。
    rows: tuple[Row, ...]
    #: 主键唯一：一个 id 一行。
    by_id: dict[int, Row]
...
class AttributeRuleTableManager:
    """AttributeRule 表的进程内单例。用 :meth:`instance` 取。"""
    ...
    @classmethod
    def instance(cls) -> "AttributeRuleTableManager":
    ...
    def build_snapshot(self, config_dir: str | Path, use_binary: bool = False) -> Snapshot:
        ...
        :param use_binary: True 读 ``attributerule.pb``（proto 二进制），
            False 读 JSON。口径与 Go/Java 的 ``useBinary`` 一致。
        """
        container = _pb.AttributeRuleTableData()
        base = Path(config_dir)
        if use_binary:
            container.ParseFromString((base / "attributerule.pb").read_bytes())
        else:
            # 文件名全小写,与 .pb 及其余五门语言同口径(json_gen.py 从前落 PascalCase,
            # 在大小写不敏感的 Windows 上碰巧能开、Linux 上必然读不到 —— 已在源头修掉)。
            path = base / "attributerule.json"
            json_format.Parse(
...
#: 模块级单例。别再 ``AttributeRuleTableManager()`` 自己新建一个 —— 那个实例是空的，
#: all_table 也不会给它加载数据。
_INSTANCE: AttributeRuleTableManager = AttributeRuleTableManager()
