"""Condition 配置表管理器（自动生成，请勿手改）。

DO NOT EDIT — regenerate from Excel via Data Table Exporter.

热更契约
========
:meth:`ConditionTableManager.load` **整批**建好一个新的不可变快照，再一次性
...
import condition_table_pb2 as _pb

#: 这张表的行类型。所有查询返回的都是它，或者它的元组。
Row = _pb.ConditionTable
...
class Snapshot:
    ...
    container: _pb.ConditionTableData
    #: 表内顺序的全部行。
    rows: tuple[Row, ...]
    #: 主键唯一：一个 id 一行。
    by_id: dict[int, Row]
    arr_condition1: dict[int, tuple[Row, ...]]
    arr_condition2: dict[int, tuple[Row, ...]]
    arr_condition3: dict[int, tuple[Row, ...]]
    arr_condition4: dict[int, tuple[Row, ...]]
...
class ConditionTableManager:
    """Condition 表的进程内单例。用 :meth:`instance` 取。"""
    ...
    @classmethod
    def instance(cls) -> "ConditionTableManager":
    ...
    def build_snapshot(self, config_dir: str | Path, use_binary: bool = False) -> Snapshot:
        ...
        by_id: dict[int, Row] = {}
        arr_condition1: dict[int, list[Row]] = {}
        arr_condition2: dict[int, list[Row]] = {}
        arr_condition3: dict[int, list[Row]] = {}
        arr_condition4: dict[int, list[Row]] = {}

        for row in rows:
            by_id[row.id] = row
            for elem in row.condition1:
                arr_condition1.setdefault(elem, []).append(row)
            for elem in row.condition2:
                arr_condition2.setdefault(elem, []).append(row)
            for elem in row.condition3:
                arr_condition3.setdefault(elem, []).append(row)
            for elem in row.condition4:
                arr_condition4.setdefault(elem, []).append(row)
        ...
        return Snapshot(
            ...
            rows=rows,
            by_id=by_id,
            arr_condition1={k: tuple(v) for k, v in arr_condition1.items()},
            arr_condition2={k: tuple(v) for k, v in arr_condition2.items()},
            arr_condition3={k: tuple(v) for k, v in arr_condition3.items()},
            arr_condition4={k: tuple(v) for k, v in arr_condition4.items()},
        )
    ...
    def find_by_condition1_index(self, key: int) -> tuple[Row, ...]:
        """反查：repeated 列 ``condition1`` 里含有 key 的全部行。"""
        return self._snap.arr_condition1.get(key, _NO_ROWS)

    def find_by_condition2_index(self, key: int) -> tuple[Row, ...]:
        """反查：repeated 列 ``condition2`` 里含有 key 的全部行。"""
        return self._snap.arr_condition2.get(key, _NO_ROWS)

    def find_by_condition3_index(self, key: int) -> tuple[Row, ...]:
        """反查：repeated 列 ``condition3`` 里含有 key 的全部行。"""
        return self._snap.arr_condition3.get(key, _NO_ROWS)

    def find_by_condition4_index(self, key: int) -> tuple[Row, ...]:
        """反查：repeated 列 ``condition4`` 里含有 key 的全部行。"""
        return self._snap.arr_condition4.get(key, _NO_ROWS)
    ...
    def count_by_condition1_index(self, key: int) -> int:
        return len(self._snap.arr_condition1.get(key, _NO_ROWS))

    def count_by_condition2_index(self, key: int) -> int:
        return len(self._snap.arr_condition2.get(key, _NO_ROWS))

    def count_by_condition3_index(self, key: int) -> int:
        return len(self._snap.arr_condition3.get(key, _NO_ROWS))

    def count_by_condition4_index(self, key: int) -> int:
        return len(self._snap.arr_condition4.get(key, _NO_ROWS))
...
#: 模块级单例。别再 ``ConditionTableManager()`` 自己新建一个 —— 那个实例是空的，
#: all_table 也不会给它加载数据。
_INSTANCE: ConditionTableManager = ConditionTableManager()
