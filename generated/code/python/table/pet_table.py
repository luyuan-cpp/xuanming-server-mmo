"""Pet 配置表管理器（自动生成，请勿手改）。

DO NOT EDIT — regenerate from Excel via Data Table Exporter.

热更契约
========
:meth:`PetTableManager.load` **整批**建好一个新的不可变快照，再一次性
...
class Snapshot:
    ...
    container: _pb.PetTableData
    #: 表内顺序的全部行。
    rows: tuple[Row, ...]
    #: 主键唯一：一个 id 一行。
    by_id: dict[int, Row]
    arr_aptitude_min: dict[int, tuple[Row, ...]]
    arr_aptitude_max: dict[int, tuple[Row, ...]]
    arr_skill: dict[int, tuple[Row, ...]]
...
class PetTableManager:
    ...
    @classmethod
    def instance(cls) -> "PetTableManager":
    ...
    def build_snapshot(self, config_dir: str | Path, use_binary: bool = False) -> Snapshot:
        ...
        :param use_binary: True 读 ``pet.pb``（proto 二进制），
            False 读 JSON。口径与 Go/Java 的 ``useBinary`` 一致。
        """
        container = _pb.PetTableData()
        base = Path(config_dir)
        if use_binary:
            container.ParseFromString((base / "pet.pb").read_bytes())
        else:
        ...
        by_id: dict[int, Row] = {}
        arr_aptitude_min: dict[int, list[Row]] = {}
        arr_aptitude_max: dict[int, list[Row]] = {}
        arr_skill: dict[int, list[Row]] = {}

        for row in rows:
            by_id[row.id] = row
            for elem in row.aptitude_min:
                arr_aptitude_min.setdefault(elem, []).append(row)
            for elem in row.aptitude_max:
                arr_aptitude_max.setdefault(elem, []).append(row)
            for elem in row.skill:
                arr_skill.setdefault(elem, []).append(row)
        ...
        return Snapshot(
            ...
            rows=rows,
            by_id=by_id,
            arr_aptitude_min={k: tuple(v) for k, v in arr_aptitude_min.items()},
            arr_aptitude_max={k: tuple(v) for k, v in arr_aptitude_max.items()},
            arr_skill={k: tuple(v) for k, v in arr_skill.items()},
        )
    ...
    def find_by_aptitude_min_index(self, key: int) -> tuple[Row, ...]:
        """反查：repeated 列 ``aptitude_min`` 里含有 key 的全部行。"""
        return self._snap.arr_aptitude_min.get(key, _NO_ROWS)

    def find_by_aptitude_max_index(self, key: int) -> tuple[Row, ...]:
        """反查：repeated 列 ``aptitude_max`` 里含有 key 的全部行。"""
        return self._snap.arr_aptitude_max.get(key, _NO_ROWS)

    def find_by_skill_index(self, key: int) -> tuple[Row, ...]:
        """反查：repeated 列 ``skill`` 里含有 key 的全部行。"""
        return self._snap.arr_skill.get(key, _NO_ROWS)
    ...
    def count_by_aptitude_min_index(self, key: int) -> int:
        return len(self._snap.arr_aptitude_min.get(key, _NO_ROWS))

    def count_by_aptitude_max_index(self, key: int) -> int:
        return len(self._snap.arr_aptitude_max.get(key, _NO_ROWS))

    def count_by_skill_index(self, key: int) -> int:
        return len(self._snap.arr_skill.get(key, _NO_ROWS))
...
#: 模块级单例。别再 ``PetTableManager()`` 自己新建一个 —— 那个实例是空的，
#: all_table 也不会给它加载数据。
_INSTANCE: PetTableManager = PetTableManager()
