"""Monster 配置表管理器（自动生成，请勿手改）。

DO NOT EDIT — regenerate from Excel via Data Table Exporter.

热更契约
========
:meth:`MonsterTableManager.load` **整批**建好一个新的不可变快照，再一次性
...
import monster_table_pb2 as _pb

#: 这张表的行类型。所有查询返回的都是它，或者它的元组。
Row = _pb.MonsterTable
...
class Snapshot:
    ...
    container: _pb.MonsterTableData
    #: 表内顺序的全部行。
    rows: tuple[Row, ...]
    #: 主键唯一：一个 id 一行。
    by_id: dict[int, Row]
...
class MonsterTableManager:
    """Monster 表的进程内单例。用 :meth:`instance` 取。"""
    ...
    @classmethod
    def instance(cls) -> "MonsterTableManager":
...
#: 模块级单例。别再 ``MonsterTableManager()`` 自己新建一个 —— 那个实例是空的，
#: all_table 也不会给它加载数据。
_INSTANCE: MonsterTableManager = MonsterTableManager()
