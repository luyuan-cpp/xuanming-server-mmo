"""World 配置表管理器（自动生成，请勿手改）。

DO NOT EDIT — regenerate from Excel via Data Table Exporter.

热更契约
========
:meth:`WorldTableManager.load` **整批**建好一个新的不可变快照，再一次性
...
class WorldTableManager:
    """World 表的进程内单例。用 :meth:`instance` 取。"""
    ...
    @classmethod
    def instance(cls) -> "WorldTableManager":
    ...
    # 外键登记（解析函数在 world_table_fk.py，不在这里）：
    # FK: scene_id -> BaseScene.id


#: 模块级单例。别再 ``WorldTableManager()`` 自己新建一个 —— 那个实例是空的，
#: all_table 也不会给它加载数据。
_INSTANCE: WorldTableManager = WorldTableManager()
