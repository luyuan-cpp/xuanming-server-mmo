"""AttributeAutoPlan 配置表管理器（自动生成，请勿手改）。

DO NOT EDIT — regenerate from Excel via Data Table Exporter.

热更契约
========
:meth:`AttributeAutoPlanTableManager.load` **整批**建好一个新的不可变快照，再一次性
...
class Snapshot:
    ...
    container: _pb.AttributeAutoPlanTableData
    #: 表内顺序的全部行。
    rows: tuple[Row, ...]
    #: 主键唯一：一个 id 一行。
    by_id: dict[int, Row]
    arr_dimension: dict[int, tuple[Row, ...]]
    arr_weight: dict[int, tuple[Row, ...]]
    idx_class_id: dict[int, tuple[Row, ...]]
    idx_pool_id: dict[int, tuple[Row, ...]]
...
class AttributeAutoPlanTableManager:
    """AttributeAutoPlan 表的进程内单例。用 :meth:`instance` 取。"""
    ...
    @classmethod
    def instance(cls) -> "AttributeAutoPlanTableManager":
    ...
    def build_snapshot(self, config_dir: str | Path, use_binary: bool = False) -> Snapshot:
        ...
        :param use_binary: True 读 ``attributeautoplan.pb``（proto 二进制），
            False 读 JSON。口径与 Go/Java 的 ``useBinary`` 一致。
        """
        container = _pb.AttributeAutoPlanTableData()
        base = Path(config_dir)
        if use_binary:
            container.ParseFromString((base / "attributeautoplan.pb").read_bytes())
        else:
            # 文件名全小写,与 .pb 及其余五门语言同口径(json_gen.py 从前落 PascalCase,
            # 在大小写不敏感的 Windows 上碰巧能开、Linux 上必然读不到 —— 已在源头修掉)。
            path = base / "attributeautoplan.json"
            json_format.Parse(
        ...
        by_id: dict[int, Row] = {}
        arr_dimension: dict[int, list[Row]] = {}
        arr_weight: dict[int, list[Row]] = {}
        idx_class_id: dict[int, list[Row]] = {}
        idx_pool_id: dict[int, list[Row]] = {}

        for row in rows:
            by_id[row.id] = row
            for elem in row.dimension:
                arr_dimension.setdefault(elem, []).append(row)
            for elem in row.weight:
                arr_weight.setdefault(elem, []).append(row)
            idx_class_id.setdefault(row.class_id, []).append(row)
            idx_pool_id.setdefault(row.pool_id, []).append(row)
        ...
        return Snapshot(
            ...
            rows=rows,
            by_id=by_id,
            arr_dimension={k: tuple(v) for k, v in arr_dimension.items()},
            arr_weight={k: tuple(v) for k, v in arr_weight.items()},
            idx_class_id={k: tuple(v) for k, v in idx_class_id.items()},
            idx_pool_id={k: tuple(v) for k, v in idx_pool_id.items()},
        )
    ...
    def find_by_dimension_index(self, key: int) -> tuple[Row, ...]:
        """反查：repeated 列 ``dimension`` 里含有 key 的全部行。"""
        return self._snap.arr_dimension.get(key, _NO_ROWS)

    def find_by_weight_index(self, key: int) -> tuple[Row, ...]:
        """反查：repeated 列 ``weight`` 里含有 key 的全部行。"""
        return self._snap.arr_weight.get(key, _NO_ROWS)

    def get_by_class_id(self, key: int) -> tuple[Row, ...]:
        """二级索引：``class_id`` == key 的全部行。"""
        return self._snap.idx_class_id.get(key, _NO_ROWS)

    def get_by_pool_id(self, key: int) -> tuple[Row, ...]:
        """二级索引：``pool_id`` == key 的全部行。"""
        return self._snap.idx_pool_id.get(key, _NO_ROWS)
    ...
    def count_by_dimension_index(self, key: int) -> int:
        return len(self._snap.arr_dimension.get(key, _NO_ROWS))

    def count_by_weight_index(self, key: int) -> int:
        return len(self._snap.arr_weight.get(key, _NO_ROWS))

    def count_by_class_id_index(self, key: int) -> int:
        return len(self._snap.idx_class_id.get(key, _NO_ROWS))

    def count_by_pool_id_index(self, key: int) -> int:
        return len(self._snap.idx_pool_id.get(key, _NO_ROWS))
    ...
    # 外键登记（解析函数在 attributeautoplan_table_fk.py，不在这里）：
    # FK: pool_id -> AttributePool.id
    # FK: dimension -> AttributeDimension.id


#: 模块级单例。别再 ``AttributeAutoPlanTableManager()`` 自己新建一个 —— 那个实例是空的，
#: all_table 也不会给它加载数据。
_INSTANCE: AttributeAutoPlanTableManager = AttributeAutoPlanTableManager()
