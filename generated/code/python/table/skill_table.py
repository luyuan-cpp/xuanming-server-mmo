"""Skill 配置表管理器（自动生成，请勿手改）。

DO NOT EDIT — regenerate from Excel via Data Table Exporter.

热更契约
========
:meth:`SkillTableManager.load` **整批**建好一个新的不可变快照，再一次性
...
class Snapshot:
    ...
    container: _pb.SkillTableData
    #: 表内顺序的全部行。
    rows: tuple[Row, ...]
    #: 主键唯一：一个 id 一行。
    by_id: dict[int, Row]
    arr_skill_type: dict[int, tuple[Row, ...]]
    arr_targeting_mode: dict[int, tuple[Row, ...]]
...
class SkillTableManager:
    """Skill 表的进程内单例。用 :meth:`instance` 取。"""
    ...
    @classmethod
    def instance(cls) -> "SkillTableManager":
    ...
    def build_snapshot(self, config_dir: str | Path, use_binary: bool = False) -> Snapshot:
        ...
        :param use_binary: True 读 ``skill.pb``（proto 二进制），
            False 读 JSON。口径与 Go/Java 的 ``useBinary`` 一致。
        """
        container = _pb.SkillTableData()
        base = Path(config_dir)
        if use_binary:
            container.ParseFromString((base / "skill.pb").read_bytes())
        else:
        ...
        by_id: dict[int, Row] = {}
        arr_skill_type: dict[int, list[Row]] = {}
        arr_targeting_mode: dict[int, list[Row]] = {}
        arr_effect: dict[int, list[Row]] = {}

        for row in rows:
            by_id[row.id] = row
            for elem in row.skill_type:
                arr_skill_type.setdefault(elem, []).append(row)
            for elem in row.targeting_mode:
                arr_targeting_mode.setdefault(elem, []).append(row)
        ...
        return Snapshot(
            ...
            rows=rows,
            by_id=by_id,
            arr_skill_type={k: tuple(v) for k, v in arr_skill_type.items()},
            arr_targeting_mode={k: tuple(v) for k, v in arr_targeting_mode.items()},
    ...
    def find_by_skill_type_index(self, key: int) -> tuple[Row, ...]:
        """反查：repeated 列 ``skill_type`` 里含有 key 的全部行。"""
        return self._snap.arr_skill_type.get(key, _NO_ROWS)

    def find_by_targeting_mode_index(self, key: int) -> tuple[Row, ...]:
        """反查：repeated 列 ``targeting_mode`` 里含有 key 的全部行。"""
        return self._snap.arr_targeting_mode.get(key, _NO_ROWS)
    ...
    def count_by_skill_type_index(self, key: int) -> int:
        return len(self._snap.arr_skill_type.get(key, _NO_ROWS))

    def count_by_targeting_mode_index(self, key: int) -> int:
        return len(self._snap.arr_targeting_mode.get(key, _NO_ROWS))
...
#: 模块级单例。别再 ``SkillTableManager()`` 自己新建一个 —— 那个实例是空的，
#: all_table 也不会给它加载数据。
_INSTANCE: SkillTableManager = SkillTableManager()
