"""GuildShop 配置表管理器（自动生成，请勿手改）。

DO NOT EDIT — regenerate from Excel via Data Table Exporter.

热更契约
========
:meth:`GuildShopTableManager.load` **整批**建好一个新的不可变快照，再一次性
...
class Snapshot:
    ...
    container: _pb.GuildShopTableData
    #: 表内顺序的全部行。
    rows: tuple[Row, ...]
    #: 主键唯一：一个 id 一行。
    by_id: dict[int, Row]
    idx_item_id: dict[int, tuple[Row, ...]]
...
class GuildShopTableManager:
    """GuildShop 表的进程内单例。用 :meth:`instance` 取。"""
    ...
    @classmethod
    def instance(cls) -> "GuildShopTableManager":
    ...
    def build_snapshot(self, config_dir: str | Path, use_binary: bool = False) -> Snapshot:
        ...
        by_id: dict[int, Row] = {}
        idx_item_id: dict[int, list[Row]] = {}

        for row in rows:
            by_id[row.id] = row
            idx_item_id.setdefault(row.item_id, []).append(row)
        ...
        return Snapshot(
            ...
            rows=rows,
            by_id=by_id,
            idx_item_id={k: tuple(v) for k, v in idx_item_id.items()},
        )
    ...
    def get_by_item_id(self, key: int) -> tuple[Row, ...]:
        """二级索引：``item_id`` == key 的全部行。"""
        return self._snap.idx_item_id.get(key, _NO_ROWS)
    ...
    def count_by_item_id_index(self, key: int) -> int:
        return len(self._snap.idx_item_id.get(key, _NO_ROWS))
    ...
    # 外键登记（解析函数在 guildshop_table_fk.py，不在这里）：
    # FK: item_id -> Item.id


#: 模块级单例。别再 ``GuildShopTableManager()`` 自己新建一个 —— 那个实例是空的，
#: all_table 也不会给它加载数据。
_INSTANCE: GuildShopTableManager = GuildShopTableManager()
