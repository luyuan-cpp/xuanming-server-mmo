"""Class 配置表管理器（自动生成，请勿手改）。

DO NOT EDIT — regenerate from Excel via Data Table Exporter.

热更契约
========
:meth:`ClassTableManager.load` **整批**建好一个新的不可变快照，再一次性
...
class ClassTableManager:
    """Class 表的进程内单例。用 :meth:`instance` 取。"""
    ...
    @classmethod
    def instance(cls) -> "ClassTableManager":
...
#: 模块级单例。别再 ``ClassTableManager()`` 自己新建一个 —— 那个实例是空的，
#: all_table 也不会给它加载数据。
_INSTANCE: ClassTableManager = ClassTableManager()
