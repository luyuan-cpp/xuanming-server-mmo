"""Mission 配置表管理器（自动生成，请勿手改）。

DO NOT EDIT — regenerate from Excel via Data Table Exporter.

热更契约
========
:meth:`MissionTableManager.load` **整批**建好一个新的不可变快照，再一次性
...
class Snapshot:
    ...
    container: _pb.MissionTableData
    #: 表内顺序的全部行。
    rows: tuple[Row, ...]
    #: 主键唯一：一个 id 一行。
    by_id: dict[int, Row]
    arr_condition_id: dict[int, tuple[Row, ...]]
    arr_next_mission_id: dict[int, tuple[Row, ...]]
    arr_target_count: dict[int, tuple[Row, ...]]
    idx_reward_id: dict[int, tuple[Row, ...]]
...
class MissionTableManager:
    """Mission 表的进程内单例。用 :meth:`instance` 取。"""
    ...
    @classmethod
    def instance(cls) -> "MissionTableManager":
    ...
    def build_snapshot(self, config_dir: str | Path, use_binary: bool = False) -> Snapshot:
        ...
        by_id: dict[int, Row] = {}
        arr_condition_id: dict[int, list[Row]] = {}
        arr_next_mission_id: dict[int, list[Row]] = {}
        arr_target_count: dict[int, list[Row]] = {}
        idx_reward_id: dict[int, list[Row]] = {}

        for row in rows:
            by_id[row.id] = row
            for elem in row.condition_id:
                arr_condition_id.setdefault(elem, []).append(row)
            for elem in row.next_mission_id:
                arr_next_mission_id.setdefault(elem, []).append(row)
            for elem in row.target_count:
                arr_target_count.setdefault(elem, []).append(row)
            idx_reward_id.setdefault(row.reward_id, []).append(row)
        ...
        return Snapshot(
            ...
            rows=rows,
            by_id=by_id,
            arr_condition_id={k: tuple(v) for k, v in arr_condition_id.items()},
            arr_next_mission_id={k: tuple(v) for k, v in arr_next_mission_id.items()},
            arr_target_count={k: tuple(v) for k, v in arr_target_count.items()},
            idx_reward_id={k: tuple(v) for k, v in idx_reward_id.items()},
        )
    ...
    def find_by_condition_id_index(self, key: int) -> tuple[Row, ...]:
        """反查：repeated 列 ``condition_id`` 里含有 key 的全部行。"""
        return self._snap.arr_condition_id.get(key, _NO_ROWS)

    def find_by_next_mission_id_index(self, key: int) -> tuple[Row, ...]:
        """反查：repeated 列 ``next_mission_id`` 里含有 key 的全部行。"""
        return self._snap.arr_next_mission_id.get(key, _NO_ROWS)

    def find_by_target_count_index(self, key: int) -> tuple[Row, ...]:
        """反查：repeated 列 ``target_count`` 里含有 key 的全部行。"""
        return self._snap.arr_target_count.get(key, _NO_ROWS)

    def get_by_reward_id(self, key: int) -> tuple[Row, ...]:
        """二级索引：``reward_id`` == key 的全部行。"""
        return self._snap.idx_reward_id.get(key, _NO_ROWS)
    ...
    def count_by_condition_id_index(self, key: int) -> int:
        return len(self._snap.arr_condition_id.get(key, _NO_ROWS))

    def count_by_next_mission_id_index(self, key: int) -> int:
        return len(self._snap.arr_next_mission_id.get(key, _NO_ROWS))

    def count_by_target_count_index(self, key: int) -> int:
        return len(self._snap.arr_target_count.get(key, _NO_ROWS))

    def count_by_reward_id_index(self, key: int) -> int:
        return len(self._snap.idx_reward_id.get(key, _NO_ROWS))
    ...
    # 外键登记（解析函数在 mission_table_fk.py，不在这里）：
    # FK: reward_id -> Reward.id


#: 模块级单例。别再 ``MissionTableManager()`` 自己新建一个 —— 那个实例是空的，
#: all_table 也不会给它加载数据。
_INSTANCE: MissionTableManager = MissionTableManager()
