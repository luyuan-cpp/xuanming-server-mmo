"""AttributeAllocRatio 配置表管理器（自动生成，请勿手改）。

DO NOT EDIT — regenerate from Excel via Data Table Exporter.

热更契约
========
:meth:`AttributeAllocRatioTableManager.load` **整批**建好一个新的不可变快照，再一次性
...
class Snapshot:
    ...
    container: _pb.AttributeAllocRatioTableData
    #: 表内顺序的全部行。
    rows: tuple[Row, ...]
    #: 主键唯一：一个 id 一行。
    by_id: dict[int, Row]
    idx_dimension_id: dict[int, tuple[Row, ...]]
...
class AttributeAllocRatioTableManager:
    """AttributeAllocRatio 表的进程内单例。用 :meth:`instance` 取。"""
    ...
    @classmethod
    def instance(cls) -> "AttributeAllocRatioTableManager":
    ...
    def build_snapshot(self, config_dir: str | Path, use_binary: bool = False) -> Snapshot:
        ...
        :param use_binary: True 读 ``attributeallocratio.pb``（proto 二进制），
            False 读 JSON。口径与 Go/Java 的 ``useBinary`` 一致。
        """
        container = _pb.AttributeAllocRatioTableData()
        base = Path(config_dir)
        if use_binary:
            container.ParseFromString((base / "attributeallocratio.pb").read_bytes())
        else:
            # 文件名全小写,与 .pb 及其余五门语言同口径(json_gen.py 从前落 PascalCase,
            # 在大小写不敏感的 Windows 上碰巧能开、Linux 上必然读不到 —— 已在源头修掉)。
            path = base / "attributeallocratio.json"
            json_format.Parse(
        ...
        by_id: dict[int, Row] = {}
        idx_dimension_id: dict[int, list[Row]] = {}
        idx_class_id: dict[int, list[Row]] = {}

        for row in rows:
            by_id[row.id] = row
            idx_dimension_id.setdefault(row.dimension_id, []).append(row)
        ...
        return Snapshot(
            ...
            rows=rows,
            by_id=by_id,
            idx_dimension_id={k: tuple(v) for k, v in idx_dimension_id.items()},
    ...
    def get_by_dimension_id(self, key: int) -> tuple[Row, ...]:
        """二级索引：``dimension_id`` == key 的全部行。"""
        return self._snap.idx_dimension_id.get(key, _NO_ROWS)
    ...
    def count_by_dimension_id_index(self, key: int) -> int:
        return len(self._snap.idx_dimension_id.get(key, _NO_ROWS))
    ...
    # 外键登记（解析函数在 attributeallocratio_table_fk.py，不在这里）：
    # FK: dimension_id -> AttributeDimension.id


#: 模块级单例。别再 ``AttributeAllocRatioTableManager()`` 自己新建一个 —— 那个实例是空的，
#: all_table 也不会给它加载数据。
_INSTANCE: AttributeAllocRatioTableManager = AttributeAllocRatioTableManager()
