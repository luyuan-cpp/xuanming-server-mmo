"""Mirror 配置表管理器（自动生成，请勿手改）。

DO NOT EDIT — regenerate from Excel via Data Table Exporter.

热更契约
========
:meth:`MirrorTableManager.load` **整批**建好一个新的不可变快照，再一次性
...
class Snapshot:
    ...
    container: _pb.MirrorTableData
    #: 表内顺序的全部行。
    rows: tuple[Row, ...]
    #: 主键唯一：一个 id 一行。
    by_id: dict[int, Row]
    idx_scene_id: dict[int, tuple[Row, ...]]
    idx_main_scene_id: dict[int, tuple[Row, ...]]
...
class MirrorTableManager:
    """Mirror 表的进程内单例。用 :meth:`instance` 取。"""
    ...
    @classmethod
    def instance(cls) -> "MirrorTableManager":
    ...
    def build_snapshot(self, config_dir: str | Path, use_binary: bool = False) -> Snapshot:
        ...
        idx_main_scene_id: dict[int, list[Row]] = {}

        for row in rows:
            by_id[row.id] = row
            idx_scene_id.setdefault(row.scene_id, []).append(row)
            idx_main_scene_id.setdefault(row.main_scene_id, []).append(row)
        ...
        return Snapshot(
            ...
            idx_main_scene_id={k: tuple(v) for k, v in idx_main_scene_id.items()},
        )
    ...
    def get_by_main_scene_id(self, key: int) -> tuple[Row, ...]:
        """二级索引：``main_scene_id`` == key 的全部行。"""
        return self._snap.idx_main_scene_id.get(key, _NO_ROWS)
    ...
    def count_by_main_scene_id_index(self, key: int) -> int:
        return len(self._snap.idx_main_scene_id.get(key, _NO_ROWS))
    ...
    # 外键登记（解析函数在 mirror_table_fk.py，不在这里）：
    # FK: scene_id -> BaseScene.id
    # FK: main_scene_id -> World.id


#: 模块级单例。别再 ``MirrorTableManager()`` 自己新建一个 —— 那个实例是空的，
#: all_table 也不会给它加载数据。
_INSTANCE: MirrorTableManager = MirrorTableManager()
