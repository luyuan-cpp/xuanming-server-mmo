"""World 的外键跨表查询助手（自动生成，请勿手改）。

DO NOT EDIT — regenerate from Excel via Data Table Exporter.

这里只做一件事：把 World 行里存的**目标表 id** 解析成目标表的行。
解析结果和源行一样属于「当前快照」，同样的热更契约：**只存 id，不存行**。

放在独立模块而不是塞进 world_table.py，是因为它要 import 目标表的
...
def get_scene_id_row(row: Row) -> _pb_basescene.BaseSceneTable | None:
    """解析 World.scene_id -> BaseScene 行；解析不到返回 ``None``。"""
...
def get_scene_id_row_by_id(table_id: int) -> _pb_basescene.BaseSceneTable | None:
    """同上，入参换成 World 自己的 id。"""
    row = WorldTableManager.instance().find_by_id(table_id)
    if row is None:
        return None
...
def find_rows_by_scene_id(key: int) -> tuple[Row, ...]:
    """反查：全部 scene_id == key 的 World 行。走的是已建好的二级索引。"""
    return WorldTableManager.instance().get_by_scene_id(key)
