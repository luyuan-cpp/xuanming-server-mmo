// Package dbtest 提供一个极小的内存版 database/sql 驱动,专供 schemamigrate 的单测使用。
//
// 复制自 go/db/internal/dbtest(internal 包跨 module 导入不了),行为保持一致。
//
// 为什么要自己写:database/sql 的 *sql.Row / *sql.Rows 都是不可从外部构造的结构体,想给
// 「拿到连接之后的那段逻辑」写单测,只能从 driver 层伪造。仓库里没有 sqlmock 之类的依赖,
// 也不打算为单测引入新依赖,所以这里做一个自包含的。
//
...
func Open(t testing.TB, h Handler) (*sql.DB, *Recorder) {
	t.Helper()
	rec := &Recorder{}
	name := fmt.Sprintf("schemamigrate-dbtest-%d", driverSeq.Add(1))
