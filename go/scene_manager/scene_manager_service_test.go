...
// 这个副本在 friend / guild / player_locator 的同名测试里逐字相同 ——
...
// etcd + Redis + 端口 + 配置表),只能靠 review 保证。
func TestKillSwitchWiredIntoUnaryChain(t *testing.T) {
	...
	// 这里刻意从生成代码里取方法名:模式写错(比如把 scene_manager 写成 scenemanager)
	// 是运维最容易犯的错,用常量拼能顺带钉住"服务名到底叫什么"。
	blocked := strings.TrimPrefix(pb.SceneManager_EnterScene_FullMethodName, "/")
	ks.SetRules(map[string]killswitch.Rule{
		blocked: {Deny: true, Reason: "单测:进场链路正在放大故障"},
	})
	...
	t.Run("命中规则的方法被短路", func(t *testing.T) {
		handlerCalled := false
		info := &grpc.UnaryServerInfo{FullMethod: pb.SceneManager_EnterScene_FullMethodName}
		h := chainUnary(chain, info, func(ctx context.Context, req any) (any, error) {
			handlerCalled = true
			return &pb.EnterSceneResponse{}, nil
		})

		resp, err := h(context.Background(), &pb.EnterSceneRequest{})
		if handlerCalled {
		...
		// "是被关停了,不是超时"。
		if !strings.Contains(st.Message(), "EnterScene") ||
			!strings.Contains(st.Message(), "进场链路正在放大故障") {
	...
	t.Run("未命中的方法照常放行", func(t *testing.T) {
		handlerCalled := false
		info := &grpc.UnaryServerInfo{FullMethod: pb.SceneManager_CreateScene_FullMethodName}
		h := chainUnary(chain, info, func(ctx context.Context, req any) (any, error) {
			handlerCalled = true
			return &pb.CreateSceneResponse{}, nil
		})

		if _, err := h(context.Background(), &pb.CreateSceneRequest{}); err != nil {
...
func TestKillSwitchFailOpenWithoutRules(t *testing.T) {
	...
	if _, err := h(context.Background(), &pb.EnterSceneRequest{}); err != nil {
