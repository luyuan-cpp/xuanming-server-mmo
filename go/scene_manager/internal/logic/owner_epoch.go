...
// 闸口推进;每一次「把玩家分配到某节点」或「放行跨 zone 交接」都推进一格,并把
// 新值随路由事件带给目标节点;C++ 存盘用 Lua 比对「缓存值 == Redis 当前值」,
// 老 epoch 的写一律被拒。Go 这边写 location 也走同一把 epoch 的 CAS,两条通道
// 不各写各的。
// ---------------------------------------------------------------------------
...
//
// 优先用旧 location 里记录的 epoch —— 那才是**仍持有玩家的节点**手里缓存的值。
// 旧 location 没有 epoch(旧版写入 / 本次之前没有 location)时退回 minted-1:
// INCR 是原子的,本次拿到 minted 就意味着写入前的值恰好是 minted-1。
func rollbackEpochFor(old *smpb.PlayerLocation, minted uint64) uint64 {
...
type handoffVerdict struct {
	...
	committed bool
	// epoch 是比对时 Redis 里的当前 owner_epoch(日志用)。
	epoch uint64
	// marker 是读到的标记原文(日志用;空串 = 没有标记)。
	marker string
}

// checkHandoffCommitted 读 player:{id}:handoff 与 player:{id}:owner_epoch,判定
// 「源 scene 是否已经把当前归属代际的最终态落地」。
//
// 判据只有一条:handoff.epoch == 当前 owner_epoch。标记里的 epoch 是源 scene 存盘
// 时持有的值,等于当前值才能证明是**当前持有者本人**在落地之后写的;小于当前值
// 是更早一代留下的旧标记(标记 EX 300s,过时的会自然消失,但比对不能靠 TTL)。
//
// 两次 GET 不是原子的,也不需要:这里只是预检,真正的串行化在后面的 Lua
// 「CAS + INCR」—— 预检通过之后若有并发请求抢先推进 epoch,本次落点会在那里
// 被拒,而不是带着过期结论写入。
//
// 标记缺失 / 写坏 / 两个键任一读失败,一律不放行(fail-closed)。读失败额外返回
// err,让调用方按 Redis 故障而不是「源未落盘」回复。
//
// epoch == 0(旧版写入的 location,侧车键从未铸造)不做特殊放宽:源 scene 手里
// 缓存的也是 0,它落盘后写出的标记就是 "0:{ms}",同样能比对通过;没有标记就
// 说明它还没落盘,与非 0 代际的语义完全一致。
func checkHandoffCommitted(svcCtx *svc.ServiceContext, playerID uint64) (handoffVerdict, error) {
	epoch, err := currentOwnerEpoch(svcCtx, playerID)
	if err != nil {
		return handoffVerdict{}, fmt.Errorf("读 owner_epoch 失败: %w", err)
	}
	markerRaw, err := svcCtx.Redis.Get(ownerepoch.HandoffKey(playerID))
	if err != nil {
		return handoffVerdict{}, fmt.Errorf("读 handoff 标记失败: %w", err)
	}
	verdict := handoffVerdict{epoch: epoch, marker: markerRaw}
	...
	verdict.committed = marker.Epoch == epoch
	return verdict, nil
}
