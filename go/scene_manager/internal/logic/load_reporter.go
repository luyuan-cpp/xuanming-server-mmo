	...
	for _, zoneId := range zones {
		...
		for _, p := range pairs {
			if _, ok := seen[p.Key]; !ok {
				...
				// SceneManager 停机 / watch 中断期间死掉的节点走不到 watch
				// DELETE,这里是唯一能观察到它们消失的地方。死亡时刻必须补记:
				// 不记的话 CanReclaimDeadNode 读不到标记,会把它们当成「没死过」
				// 直接放行改派,而它们很可能是几秒前刚断的、C++ 侧还在 drain。
				// 我们不知道真实死亡时刻,只能用「首次观察到」这个偏晚的时刻 ——
				// 方向是安全的(屏障结束点跟着偏晚)。
				markNodeDeath(svcCtx, zoneId, p.Key)
				// 计数残留同样要在这里清(理由见 deleteNodeCounters)。
