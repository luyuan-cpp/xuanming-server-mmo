...
// backfillPlayerClass 只在首次入场前补齐旧存档漏存的职业。
// 热缓存、子表缓存、DB 加载都汇入 applyLoadedPlayerSession，所以不会漏掉缓存命中。
// 已有会话的角色仍由 scene 持有权威数据；旧账号职业为零时也不猜测职业。
func (l *EnterGameLogic) backfillPlayerClass(ctx context.Context, existing *sessionmanager.PlayerSession, state enterGameSessionState) error {
    if existing != nil || state.classID == 0 {
        return nil
    }
    ...
    if state.playerLockKey == "" || state.playerLockToken == "" {
        return fmt.Errorf("职业补齐缺少玩家登录锁: player=%d", state.playerID)
    }
    ...
    if err != nil {
        return fmt.Errorf("读取职业补齐存档: %w", err)
    }
    if err := proto.Unmarshal(previous, parent); err != nil {
        return fmt.Errorf("解析职业补齐存档: %w", err)
    }
    player := parent.GetPlayerDatabaseData()
    if player == nil || player.GetPlayerId() != state.playerID {
        return fmt.Errorf("职业补齐存档与玩家不匹配: player=%d", state.playerID)
    }
    if player.GetUint32PbComponent().GetClass() != 0 {
        return nil
    }
    ...
    if err != nil {
        return fmt.Errorf("序列化职业补齐存档: %w", err)
    }
    ...
    if err != nil {
        return fmt.Errorf("保存职业补齐存档: %w", err)
    }
    switch result {
    case 1, 2: // 成功，或新的会话/场景仍持有权威数据而跳过。
        return nil
    case -1:
        return fmt.Errorf("职业补齐时玩家登录锁已失效: player=%d", state.playerID)
    default:
        return fmt.Errorf("职业补齐时存档已变化，请重试入场: player=%d", state.playerID)
    }
}
