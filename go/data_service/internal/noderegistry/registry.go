// Package noderegistry handles C++ convention etcd registration for DataService.
// C++ nodes discover services by watching etcd key prefixes like
// "DataServiceNodeService.rpc/zone/{zoneId}/node_type/{nodeType}/node_id/{nodeId}".
// The value is a protojson-serialized NodeInfo message.
//
// 为什么 data_service 需要这一层:go-zero 只往 etcd 写自己的 `dataservice.rpc` 发现键,
// C++ 侧(cpp/libs/engine/core/node/system/etcd)只认上面那种 NodeInfo 路径。scene 经
// 这条路径发现 data_service 并调 AllocateIdSegment 领 item / txlog / snapshot 号段
// (docs/design/node-id-overhaul-plan-20260908.md §6.4 Phase 5),发现不到就永远过不了
// DependencyGate。DataServiceNodeService **不是** zone-scoped 节点类型(C++ 发现侧只有
// Gate/Scene/Login/PlayerLocator 按 zone 过滤),路径里的 zone 只是排障时对得上号。
package noderegistry

// 本包是 go/scene_manager/internal/noderegistry/registry.go 的逐字副本(scene_manager 的
// internal 包跨 module 引不到),只改了 safego 点位名、日志前缀与本段注释。
// 同样的 allocator 逻辑在 guild / friend / player_locator / scene_manager / match 里也各有一份;
// 若需修改 allocator 行为,请同步修改:
//   - go/scene_manager/internal/noderegistry/registry.go(本文件的来源)
//   - go/match/internal/noderegistry/registry.go
...
const safePointNodeKeepAlive = "data_service.node_keepalive"
...
// rpcPrefix returns the etcd key prefix for a given node type, e.g. "DataServiceNodeService.rpc".
...
// 因为两个 zone 的实例不可能同时拿到同一个 (node_type, node_id)。
...
func allocateNodeID(ctx context.Context, cli *clientv3.Client, prefix string, info *base.NodeInfo, leaseID clientv3.LeaseID) (uint32, error) {
	...
	for id := nodeIDMin; id <= nodeIDMax; id++ {
		...
		if err != nil {
			logx.Errorf("data_service allocator: txn failed for node_id=%d: %v", id, err)
			continue
		}
		if ok {
			return id, nil
...
func (nr *NodeRegistration) KeepAlive() {
	...
	// 兜住之后至少能在 safego_panic_total{point="data_service.node_keepalive"}
	// 上看见,并知道本进程已经失去续租能力。
...
//	抢占了同一个 node_id。此时盲目 Put 老 key 会覆盖别人的 NodeInfo,
//	C++ watcher 把调用路由到错误节点。
//
// 流程:
//  1. 拿新 lease。
...
//     这恰好覆盖了 "别人没抢" 这个语义。
//  3. CAS 失败 → 原 node_id 已被其他 uuid 占用,绝不覆盖,改调 allocateNodeID
//     拿全新 node_id。data_service 没有 "我必须是 node N" 的持久化语义
//     (号段水位在 MySQL id_segment 表里,与 node_id 无关),换 ID 是安全的。
