// Package noderegistry handles C++ convention etcd registration for Match.
// C++ nodes discover services by watching etcd key prefixes like
// "MatchNodeService.rpc/zone/{zoneId}/node_type/{nodeType}/node_id/{nodeId}".
// The value is a protojson-serialized NodeInfo message.
package noderegistry

// 本文件中的 allocator 逻辑在 guild / friend / player_locator / scene_manager /
// match 五个服务里是逐字相同的副本(只有 import 包名和 reRegister 行为不同)。
// 若需修改 allocator 行为,请同步修改另外四个服务的对应文件:
...
// rpcPrefix returns the etcd key prefix for a given node type, e.g. "MatchNodeService.rpc".
...
func allocateNodeID(ctx context.Context, cli *clientv3.Client, prefix string, info *base.NodeInfo, leaseID clientv3.LeaseID) (uint32, error) {
	...
	for id := nodeIDMin; id <= nodeIDMax; id++ {
		...
		if err != nil {
			logx.Errorf("match allocator: txn failed for node_id=%d: %v", id, err)
			continue
		}
		if ok {
			return id, nil
...
//     allocateNodeID 拿全新 node_id。match 没有 "我必须是 node N" 的
//     持久化语义,换 ID 是安全的。
