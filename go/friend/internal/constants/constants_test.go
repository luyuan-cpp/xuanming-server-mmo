...
// TestTipCodesStayInFriendSegment 钉死「好友 tip 码只能落在自己的号段里」。
func TestTipCodesStayInFriendSegment(t *testing.T) {
	for name, code := range scanTipCodes(t) {
		if code < uint64(FriendTipCodeLo) || code > uint64(FriendTipCodeHi) {
			t.Errorf("%s = %d 越出好友号段 [%d, %d]", name, code, FriendTipCodeLo, FriendTipCodeHi)
		}
	}
}

// TestTipCodesDoNotCollideWithGeneratedTipTable 是本次修复的回归护栏:
// 好友码曾经从 1 开始手写,与 common 段(kInvalidTableId=2 / kInvalidTableData=3 /
// kServiceUnavailable=4)整段重叠,客户端按 id 查提示表会显示完全无关的文案。
//
...
func TestTipCodesDoNotCollideWithGeneratedTipTable(t *testing.T) {
	...
	collided := []struct {
		...
		{"kServiceUnavailable", uint32(table.CommonError_kServiceUnavailable)},
	}
...
// TestTipClassifier 覆盖定性函数的两条分支:段内一律业务拒绝、段外交回全局判定。
func TestTipClassifier(t *testing.T) {
	classify := TipClassifier()

	for _, code := range []uint32{
		ErrCannotAddSelf, ErrAlreadyFriends, ErrFriendListFull,
		ErrRequestAlreadySent, ErrTargetFriendListFull,
		ErrNoPendingRequest, ErrTooManyPending,
	} {
