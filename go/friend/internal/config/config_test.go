...
// TestEtcYamlEnablesPrometheus 钉死 etc/friend.yaml 里的 Prometheus 段确实能被解析、
// 并且真的会启用 /metrics。
//
// 这条是「好友服务完全没有 /metrics」那个可观测性缺口的回归护栏:
...
func TestEtcYamlEnablesPrometheus(t *testing.T) {
	var c Config
	if err := conf.Load("../../etc/friend.yaml", &c); err != nil {
		t.Fatalf("加载 etc/friend.yaml 失败: %v", err)
	}
	...
	if c.Prometheus.Port != 9180 {
		t.Errorf("Prometheus.Port = %d,好友服务约定用 9180", c.Prometheus.Port)
	}
}
