// Package lifecycle 协调 friend 进程的正常停机:注销 → 排空 RPC → 关业务资源 → 框架收尾,全程有界。
//
...
//
// 来源:照抄 go/trade/internal/lifecycle(2026-09-17;trade 那份又照搬自 go/chat,是目前唯一带完整
// 单测的版本),只改了包注释里的服务名。常量值、算式与机制注释一律保持原样 —— 它们记录的是
// 已经踩过的坑,擅自"优化"等于把坑再踩一遍。
//
// 这是第 4 份临时副本:chat / guild / trade / friend 各有一份。定稿后应抽到 go/shared 统一替换;
// 在那之前四份必须同改(只改一份 = 副本漂移,停机顺序会在某个服务上静默退化)。
package lifecycle
