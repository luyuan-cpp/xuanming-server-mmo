...
// 这个副本在 guild / player_locator / scene_manager 的同名测试里逐字相同 ——
...
func TestKillSwitchWiredIntoUnaryChain(t *testing.T) {
	...
	// 这里刻意从生成代码里取方法名:模式写错(比如把 friendpb 写成 friend)
	// 是运维最容易犯的错,用常量拼能顺带钉住"服务名到底叫什么"。
	blocked := strings.TrimPrefix(pb.FriendService_AddFriend_FullMethodName, "/")
	...
	t.Run("命中规则的方法被短路", func(t *testing.T) {
		handlerCalled := false
		info := &grpc.UnaryServerInfo{FullMethod: pb.FriendService_AddFriend_FullMethodName}
		h := chainUnary(chain, info, func(ctx context.Context, req any) (any, error) {
			handlerCalled = true
			return &pb.AddFriendResponse{}, nil
		})

		resp, err := h(context.Background(), &pb.AddFriendRequest{})
		if handlerCalled {
		...
		// "是被关停了,不是超时"。
		if !strings.Contains(st.Message(), "AddFriend") ||
			!strings.Contains(st.Message(), "好友库过载") {
	...
	t.Run("未命中的方法照常放行", func(t *testing.T) {
		handlerCalled := false
		info := &grpc.UnaryServerInfo{FullMethod: pb.FriendService_GetFriendList_FullMethodName}
		h := chainUnary(chain, info, func(ctx context.Context, req any) (any, error) {
			handlerCalled = true
			return &pb.GetFriendListResponse{}, nil
		})

		if _, err := h(context.Background(), &pb.GetFriendListRequest{}); err != nil {
...
func TestKillSwitchFailOpenWithoutRules(t *testing.T) {
	ks := killswitch.New(killswitch.Config{})
	// 刻意不调 SetRules,也不调 Start —— 模拟 etcd 客户端为 nil 的部署。
	...
	if _, err := h(context.Background(), &pb.AddFriendRequest{}); err != nil {
