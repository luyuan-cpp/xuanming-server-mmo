...
func (n *Node) Generate() (uint64, error) {
	...
	default:
		...
		//
		// ⚠️ guard 预置的这一次**必须走下面的真实等待,绝不能改成"直接借 guard+1"**
		// (2026-08-11 试过,TestNewNode_BootGuardSkipsRestartSecond 当场抓出重号):
		// 这次等待不是低效,而是**跨重启屏障的一部分** —— 它强迫真实时间流逝越过
		// guard 秒,于是本进程发出的号永远落在"墙钟已真实到达"的秒里;下一任(亚秒
		// 重启 / hostname 亲和拿回同 id)用墙钟当 guard 地板时才罩得住前任全部发号秒。
		// 借位会把号发到墙钟前面,新任的墙钟地板罩不住它,同秒重启即逐位重号。
		// 代价只是"激活同一秒内就有人发号"时那一次 <1s 的等待,现实中几乎不可达
		// (首个业务发号通常远晚于激活)。
		if !guardPending {
			behind := uint64(0)
			if now < n.lastTime {
