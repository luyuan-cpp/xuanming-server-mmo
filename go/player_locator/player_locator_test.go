...
// 这个副本在 friend / guild / scene_manager 的同名测试里逐字相同 ——
...
func TestKillSwitchWiredIntoUnaryChain(t *testing.T) {
	...
	// 这里刻意从生成代码里取方法名:模式写错(比如把 playerlocator 写成 player_locator)
	// 是运维最容易犯的错,用常量拼能顺带钉住"服务名到底叫什么"。
	blocked := strings.TrimPrefix(pb.PlayerLocator_GetLocation_FullMethodName, "/")
	ks.SetRules(map[string]killswitch.Rule{
		blocked: {Deny: true, Reason: "单测:位置查询把 Redis 打爆了"},
	})
	...
	t.Run("命中规则的方法被短路", func(t *testing.T) {
		handlerCalled := false
		info := &grpc.UnaryServerInfo{FullMethod: pb.PlayerLocator_GetLocation_FullMethodName}
		h := chainUnary(chain, info, func(ctx context.Context, req any) (any, error) {
			handlerCalled = true
			return &pb.PlayerLocation{}, nil
		})

		resp, err := h(context.Background(), &pb.PlayerId{})
		if handlerCalled {
		...
		// "是被关停了,不是超时"。
		if !strings.Contains(st.Message(), "GetLocation") ||
			!strings.Contains(st.Message(), "把 Redis 打爆了") {
	...
	t.Run("未命中的方法照常放行", func(t *testing.T) {
		handlerCalled := false
		info := &grpc.UnaryServerInfo{FullMethod: pb.PlayerLocator_GetSession_FullMethodName}
		h := chainUnary(chain, info, func(ctx context.Context, req any) (any, error) {
			handlerCalled = true
			return &pb.GetSessionResponse{}, nil
		})

		if _, err := h(context.Background(), &pb.GetSessionRequest{}); err != nil {
...
func TestKillSwitchFailOpenWithoutRules(t *testing.T) {
	...
	if _, err := h(context.Background(), &pb.PlayerId{}); err != nil {
