...
func commonSegment(t *testing.T) tip.Segment {
	t.Helper()
...
// 路由服复用 common 段的码:必须落在 common 段内且被段表识别。
func TestTipCodesStayInCommonSegmentAndAreRecognized(t *testing.T) {
	segment := commonSegment(t)
	for name, code := range tipCodes() {
		if code < segment.Base || code >= segment.Base+segment.Width {
			t.Errorf("%s = %d 落在 common 段 [%d,%d) 之外", name, code,
...
// 与 match 的同名测试同一机制:Err 常量必须直接写成 uint32(table.CommonError_kX)。
	...
	ast.Inspect(file, func(node ast.Node) bool {
		...
		for index, name := range valueSpec.Names {
			...
			if !isGeneratedTipRef(valueSpec.Values[index], "CommonError_") {
				t.Errorf("%s 必须直接写成 uint32(table.CommonError_kX),不能手写数字、别名或引用其他码轴",
					name.Name)
			}
		}
		return true
