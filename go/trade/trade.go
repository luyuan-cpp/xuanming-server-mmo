// trade 服务 P1:聚宝斋浏览 / 详情 / 收藏 / 货架(客户端经 gate → 路由服到达)与内部 TradeAdmin.SeedListing。
//
// 全局一份、多副本、进程无状态:数据在独占库 mmorpg_trade(port-decisions D-14),玩家 home_zone 与
// listing_id 号段来自 data_service。注册按 C++ NodeInfo 约定写进 etcd(shared/noderegistry),
// 路由服按 TradeNodeService.rpc 前缀发现本服务(契约 microservice-zone-contract §2/§3)。
//
// 两种运行形态:
//   - 常驻服务(默认):建表 / 查表 → metrics → killswitch → gRPC → 注册 etcd;
//   - `-migrate`:只连 MySQL 跑一次 schemamigrate.Up,按 D-14 退出码退出(K8s trade-migrate Job 用),
//     不起 gRPC、不连 etcd、不连 data_service。
package main
...
var (
	...
	// migrateOnly 是生产用的显式建表入口(D-14 第 4 条):`trade -f <yaml> -migrate` 只对 mmorpg_trade
...
const nodeType = base.ENodeType_TradeNodeService

const (
	// listenProbeTimeout:等本进程 gRPC 端口可连的上限。先探端口再注册,是为了不让路由服
	// 在监听建立前就选中本节点(拨号失败 = 客户端收到 kServiceUnavailable)。与 chat 同值。
...
const migrateRemedyCommand = "trade -f etc/trade.yaml -migrate"
...
//
// 停机(契约 §2、§14.1 第 5 条、§16;照 chat 已活体验证的做法):
...
//     → ③ 停 killswitch watch、关号段 / DB / etcd → ④ proc.Shutdown 与等 Start 返回(≤2s)→ 刷日志。
//   - 到 24s 仍未收尾由 watchHardDeadline 强制退出:不保证在途请求完成;注销传输失败仍靠 LeaseTTL 兜底。
func runTrade(c config.Config) (runErr error) {
	...
	defer func() {
		...
		if err := shutdown.Stop(); err != nil {
			logx.Errorf("[trade] 停机收尾未按预算完成: %v", err)
			runErr = errors.Join(runErr, err)
		}
		_ = logx.Close()
	}()

	// 建表 / 查表(D-14 第 4 条)。放在起 gRPC 之前:表不对的 trade 一个请求都不该接。
	...
	s, err := zrpc.NewServer(c.RpcServerConf, func(grpcServer *grpc.Server) {
		// 两个服务挂在同一个 gRPC server 上:客户端协议与内部 TradeAdmin 的隔离靠会话拦截器的
		// 方法白名单(internal/session),以及 gate / 路由服不认 TradeAdmin 的消息号。
		tradepb.RegisterClientPlayerJubaozhaiServer(grpcServer, server.NewJubaozhaiServer(deps))
		tradepb.RegisterTradeAdminServer(grpcServer, server.NewAdminServer(deps))
	...
	if err != nil {
		return fmt.Errorf("trade RPC 构造失败: %w", err)
	}
	...
	case <-ctx.Done():
		return nil
	case err := <-serveErr:
		return fmt.Errorf("trade gRPC 监听未就绪: %v", err)
	}
	...
	nr, err := noderegistry.RegisterAfterListening(regCtx, svcCtx.Etcd, dialAddress(listenHost, port), listenProbeTimeout,
		noderegistry.Spec{
			// Prefix 由枚举名派生,不手写(契约 §2):"TradeNodeService.rpc"。
			...
			// trade 不以 node_id 派生任何持久身份(listing_id 来自号段,不是 snowflake),失租后换 id 继续服务即可。
			OnReclaimFailed: noderegistry.ReallocateNewID,
			OnNodeIDChanged: func(oldID, newID uint32) {
				logx.Errorf("[trade] etcd 失租后原 node_id=%d 未能重夺,已换新 node_id=%d 继续服务", oldID, newID)
			},
			LogPrefix: "[trade]",
		})
	regCancel()
	if err != nil {
		...
		return fmt.Errorf("trade 节点注册 etcd 失败: %w", err)
	}
	shutdown.Unregister = nr.Close
	nr.KeepAlive()

	fmt.Println("\n=============================================================")
	fmt.Println("  TRADE SERVICE STARTED SUCCESSFULLY")
	fmt.Println("=============================================================")
	fmt.Printf("  Listen:        %s\n", c.ListenOn)
	...
	select {
	case <-ctx.Done():
		logx.Info("[trade] 收到退出信号,先注销再排空 RPC")
	case err := <-serveErr:
		return fmt.Errorf("trade gRPC 服务提前退出: %v", err)
	}
...
// finished 在 runTrade 返回时关闭。go-zero 在 24s 时会给自己补发一次信号,但本进程的
...
func watchHardDeadline(ctx context.Context, stopSignals context.CancelFunc, finished <-chan struct{}) {
	...
	case <-finished:
	case <-timer.C:
		fmt.Fprintf(os.Stderr, "[trade] 已到 %v 停机硬截止,强制退出;不能确认在途请求已完成\n", lifecycle.HardTimeout)
		os.Exit(1)
	}
}
...
// 抽成函数是为了让链本身可测(trade_test.go),理由同 chat / guild。
func buildUnaryInterceptors(ks *killswitch.Switch) []grpc.UnaryServerInterceptor {
	return []grpc.UnaryServerInterceptor{
		...
		// ② 热关停紧跟其后,尽早短路:命中后会话解码、in-band 定性、MySQL / data_service 访问统统不做。
		ks.UnaryServerInterceptor(),

		// ③ 会话与方法准入(guild 式,P1-4):无会话 = 内部调用放行;坏会话 → Unauthenticated;
		//    带会话调白名单外方法(TradeAdmin.*)→ PermissionDenied。放在 serverbase 之前:
...
func ensureSchema(ctx context.Context, db *sql.DB, c config.Config, up, plan schemaRunner) error {
	...
	if c.ShouldAutoMigrate() {
		...
		if err != nil {
			return fmt.Errorf("[trade] 启动期建表失败(库 %s),拒绝启动: %w", data.DatabaseName, err)
		}
		if len(report.Manual) > 0 {
			return fmt.Errorf("[trade] 启动期建表发现 %d 项需人工处理(库 %s),拒绝启动: %s",
	...
	if err != nil {
		return fmt.Errorf("[trade] Schema.AutoMigrate=false,启动期检查表结构失败(库 %s),拒绝启动;确认库可达后执行 %s: %w",
			data.DatabaseName, migrateRemedyCommand, err)
	}
	if !report.Clean() {
		return fmt.Errorf("[trade] Schema.AutoMigrate=false 且库 %s 与 proto/trade/trade_table.proto 不一致"+
...
func runMigration(c config.Config, allowModify bool, openMySQL func(config.MySQLConf) (*sql.DB, error), up schemaRunner) int {
	...
	default:
		fmt.Printf("schema migration OK: %s synced from proto/trade/trade_table.proto (%d statement(s) executed)\n",
...
func logReport(stage string, r schemamigrate.Report) {
	for _, s := range r.Statements {
		logx.Infof("[trade] %s statement: %s", stage, s)
	}
	for _, w := range r.Warnings {
		logx.Errorf("[trade] %s warning: %s", stage, w)
	}
	for _, m := range r.Manual {
...
// shared 不 import proto 模块(契约 §2),所以 NodeInfo 由调用方拼;字段填法与 chat.go 相同。
// trade_test.go 的 TestNodeInfoValueMatchesRegistryContract 钉住注册前校验要求的字段。
...
// advertisedHost 决定写进 NodeInfo 的 IP(与 chat.go advertisedHost 同一口径):
