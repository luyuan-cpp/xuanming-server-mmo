...
type ItemEntry struct {
	...
	// 入包序号(同一背包内单调递增,越小越早进包)。镜像 ItemComp.acquire_seq。
	//
	// 淘汰策略(临时格先进先出)按它排序;不持久化的话,跨服一跳顺序就丢了,
	// 而 item_uuid 顶替不了它 —— 跨服迁移的号是源服 node 铸的、邮件附件的号是
	// 调用方预设的,两者都不反映本包内的先后。
	//
	// 字段号从 13 起:6–12 已被上面那张 TODO 表预定,占用它们会让日后加那些
	// 字段时撞号。0 = 旧存档没盖过章,还原后按 item_uuid 排序(与本字段落地前
	// 的行为一致),并由 ItemStore 从现有实例重建水位。
	AcquireSeq    uint64 `protobuf:"varint,13,opt,name=acquire_seq,json=acquireSeq,proto3" json:"acquire_seq,omitempty"`
...
func (x *ItemEntry) GetAcquireSeq() uint64 {
	if x != nil {
		return x.AcquireSeq
	}
	return 0
...
type BagAllData_DynamicBagData struct {
	...
	// 这个动态包按哪套规则装配(布局 / 准入 / 淘汰),见
	// cpp/libs/modules/bag/bag_profile_registry.h。
	//
	// **必须持久化。** 规则本身是代码 + 配置,不进快照;但"这个包是哪套规则"
	// 是玩家数据。不带它,跨服回来节日包会退化成默认的自由格 + 什么都收 ——
	// 规则静默消失,节日包变成普通包。
	// 0 = 未指定,按默认 profile 还原(与本字段落地前的行为一致)。
...
func (x *BagAllData_DynamicBagData) GetProfileId() uint32 {
	if x != nil {
		return x.ProfileId
	}
	return 0
...
const file_proto_common_database_bag_quest_mail_data_proto_rawDesc = "" +
	...
	"\bbag_type\x18\x05 \x01(\rR\abagType\x12\x1f\n" +
	"\vacquire_seq\x18\r \x01(\x04R\n" +
	"acquireSeq\"\x94\x02\n" +
	"\n" +
	"BagAllData\x12 \n" +
	...
	"capacities\x12=\n" +
	"\fdynamic_bags\x18\x03 \x03(\v2\x1a.BagAllData.DynamicBagDataR\vdynamicBags\x1a\x84\x01\n" +
	...
	".ItemEntryR\x05items\x12\x1d\n" +
	"\n" +
	"profile_id\x18\x04 \x01(\rR\tprofileId\"\x81\x01\n" +
	"\n" +
	"QuestEntry\x12\x1b\n" +
