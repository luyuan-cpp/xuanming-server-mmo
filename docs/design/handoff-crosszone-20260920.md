...
> **验证现状(2026-09-21 更新)**:C++ 已由 Codex 按 `msbuild game.sln /m:1 /nr:false /p:Configuration=Debug /p:Platform=x64` 整体编译通过(退出码 0,见 PROGRESS.md 同日 Codex 条目);`cross_zone_test`、`routing_identity_test` 都在 `game.sln` 里,**编译过了但没运行**。实际跑过的只有 `bag_test` 的 10 个结算用例。**Go(`go/scene_manager`、`go/login`)的 build / vet / test、Unity 编译与 EditMode 测试、任何联调与故障注入都还没有证据**。任何"已修"都应读作"已落码 / 已编译,待测试"。**那次编译早于同日落码的 Z1 / A′ / GO-5 / epoch==0 铸造(提交 `9cef7b2ec` 及其后工作树增量),这批 C++ 代码没有编译证据,Go 同样没有(§4 的 0g)。**
...
2. **§12.3 的 P2 五条**,每条都写了"为什么没修 / 修法":
   ...
   - 客户端侧:CL-3 / CL-4(回家入口消失)已随 §12.5.5 修复;~~CL-2 仍未修~~ **CL-2 已落码待验证**(2026-09-21,`ValidateRedirectTarget` 只打日志不拦截,以 gate(B) 验票为准,§12.5.6);CL-7 已入库;CL-5(手动重进无冷却)的根因修复见 0d(已落码待验证,见 0g)。
