...
**背景与证据**
`turn_battle_engine.cpp:511-524` FillDefaultActions 对怪物一律 BATTLE_ACTION_ATTACK、target 0 随机;`data/schema/monster_table.proto:17-27` 无技能列;`generated/tables/Skill.json` 首行 damage='100*level'(`skill_table.proto:78`);Class 表 init_speed=20(`class_table.proto:26`),`player-attribute-allocation.md` §8 记速度 1 级 23 > 全部怪物 8~20、体质 5 点让 1 号怪普攻归零(ApplyDamage `turn_battle_engine.cpp:1047-1056` 饱和到 0);PROGRESS.md:3707「记入缺口不擅改表(策划平衡项)」。PVE 已数据化到 15~24 回合,但怪物永远后手、逃跑 95% 饱和、低级投体质即无敌。**2026-09-10 更新**:角色属性系数已由策划上调(体质 50 血 + 5 防 / 灵力 40 法伤 + 10 蓝 / 力量 50 物伤 / 敏捷 3 速度),上述"5 点""15~24 回合"都是旧系数口径——新系数下 1 级投 2 点体质(或 3 级不投点)即让 1 号怪普攻归零,低级副本 1~2 回合结束,PVP 7 级起首击秒杀;最新定量见 `player-attribute-allocation.md` §8。本条剩余待决的是怪物侧强度。**2026-09-11**:怪物侧已按新系数重定 `Monster.xlsx` 1-16 的血量 / 力量 / 速度(设计目标与推算见 `player-attribute-allocation.md` §8,静态推算,未实测);相性 / 仙魔 / 宝宝系数已同比上调。仍待决:PVP 同级首击秒杀(需引擎的 PVP 伤害规则)。**2026-09-13/14**:伤害改比例减伤(`combat_damage_rules.h`,不再归零、常驻减伤封顶 60%);PVP 加 `kPvpDamageScale`(09-14 为 0.3,同级 30 级以上约 5~6 下);相性 / 仙魔两池已删;速度单位 ×12(`Class.init_speed` 240、逃跑系数同除 12);怪物按最终口径再重定,普通怪速度约为同级玩家 0.9 倍(见 `player-attribute-allocation.md` §8,模拟推算、未实测)。本条剩余待决:怪物技能 / AI(D-04 ②)与 `Skill.damage` 表达式(③);上面的验收标准(归零阈值、逃跑 95% 饱和)以旧口径写成,领取前按 §8 重写。
