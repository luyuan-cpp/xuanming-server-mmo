...
- **行由迁移显式创建**(`IdSegment.BootstrapTags`,没配 = `player, guild, item, txlog, snapshot, trade_listing, guild_asset_op`;`trade_listing` 是聚宝斋商品号,2026-09-14 加入;`guild_asset_op` 是帮会资产指令 op_id,2026-09-20 帮会二期 B5a 加入,消费表在独占库 `mmorpg_guild`、不做地板校验):`-migrate` 与 AutoMigrate 都在四张表就位之后对每个 tag `INSERT IGNORE ... (max_id=1, step=100, version=0)`,幂等,**绝不降低**已有行的 max_id;每行创建 / 已存在各打一条 Info。清单里出现非法 tag(不匹配 `^[a-z0-9_]{1,64}$`)整次迁移失败。新增一种永久身份 = 加一个 tag。
...
IdSegment:
  ...
  BootstrapTags: [player, guild, item, txlog, snapshot, trade_listing, guild_asset_op]   # 没配 = 这份默认清单;迁移 INSERT IGNORE 预建,绝不降位
Kafka:
