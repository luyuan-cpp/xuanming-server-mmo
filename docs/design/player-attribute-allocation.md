...
- **`AttributeDimension`** —— 维度 + **系数矩阵**:`pool_id`(idx)、`name` / `desc`(直接下发客户端做面板文案与悬浮说明)、
  ...
  体质每点 +50 气血上限、+5 防御;灵力每点 +40 法伤、+10 法力上限;
  ...
  这条"同比换算"规则 2026-09-13 起不再适用:角色分配点已不是每点系数,相性 / 仙魔要不要也切到百分比见 §8。
...
- **`AttributeAllocRatio`**(2026-09-13 新增)—— 角色属性点的**加点收益比例**:`dimension_id`(idx,fk)× `class_id`
  ...
  **只覆盖角色属性点池(101-104)**。相性 / 仙魔 / 宝宝池没有比例行,继续按 `AttributeDimension` 每点固定加值
  (用户 2026-09-13 选定)。将来要把相性 / 仙魔切到百分比,给它们的维度加行即可:满投点数按该维度所属池现算
  (相性单项上限 50、仙魔满级 26 点),不会被角色属性点的 425 尺度压小。
- **`AttributeAutoPlan`** —— 自动加点方案:`class_id`(idx,0 = 通用兜底)× `pool_id` × `dimension[]` 优先序 + `weight[]`。
  ...
  而全投力量 +1062.5 物伤),推荐方案不该系统性劣于公式意图。相性 / 仙魔池方案不变。
...
- **满投正好拿满比例**:n = 425 时 E = 510 = d,增量 = 标准基础 × 比例。例如非破军力量满投 = 4250 × 25% = +1062.5 物伤,
  破军 = +1275;玄霄灵力满投 = 3400 × 27% = +918 法伤 + 1050 × 15% = +157.5 法力。
...
- 没有比例行的维度(相性 / 仙魔 / 宝宝)保持每点固定加值,代码路径 `AddLinear`。纯规则在
...
- **面板 `value` 只是点数,不能反推收益**:85 级力量 170 = 85 点自然成长(值 +4250 物伤)+ 85 点分配(值 +184)。
  分配点的单点边际很小:速度、防御每点约 +0.09,要连投 12 点才看到 +1;法力每点约 +0.31,4 点才 +1
  (二级属性是整数,向下取整)。这是策划公式本身的性质,不是代码误差;玩家可能报"加了点没生效",见 §8。
...
受伤比例 = max(0.4, 等级系数 ÷ (护甲 + 防御 + 等级系数) × (1 − 抗性%)),等级系数 = 30 + 10 × 目标等级(夹到 1..85)
伤害     = 原始伤害 × 受伤比例
          回合制非 PVE 对局再 × kPvpDamageScale(0.2);然后 critchance/100 概率 ×2;DEFEND 再减半;向上取整、不超过当前气血
```
...
- **`class_id` 未随 `PlayerAllData` 下发 scene**:`ResolveClassRow` / `AutoAllocate` / 2026-09-13 的 `FindAllocRatio`
  三处职业分支都已写好,但 scene 拿到的 `PlayerUint32Comp.class` 目前仍是 0 —— **加点公式在职业打通前全部走
  `class_id=0` 的非对应职业档**,破军练力量拿不到 +30%。并行会话 2026-09-13 正在 login 侧补 `backfillPlayerClass`
  (未提交),那条路通了之后这里自动生效,无需再改。
...
- **2026-09-13 加点公式切换后的平衡口径整体过期**:下面 ① ~ ③ 的推算都基于"每点 +50 物伤"的旧口径。新口径下
  加点收益缩到旧的约 1/20(破军 1/17;力量满投 +1062.5~1275 而不是 +21250)。以 ① 所参照的 30 级通用号为例
  (属性点自动加点 + 相性先满土):旧口径物伤 ≈ 7800 = (30 自然 + 90 分配)× 50 + 土相 30 × 60;
  新口径 ≈ 3635 = 1500 自然 + 335 加点(自动加点现全投力量 150 点)+ 1800 土相 —— **土相是力量加点的 5 倍多,
  这个号的输出主要来自相性**。**以上都是静态推算,未经战斗实测**。
  **2026-09-13 用户拍板后已处理**(静态推算口径 = 新加点公式 + §3.2 比例减伤 + 通用自动加点,不算暴击,职业未打通按兜底档):
  - 伤害公式改为比例减伤(§3.2);PVP(回合制非 PVE 对局)直接伤害 × 0.2:同级通用 1v1 在 7 / 10 / 30 / 60 / 85 级分别
    约 8 / 7 / 5 / 5 / 5 下打死(全暴击约 3~4 下),改前 30 级以上一下打死。
  - `Monster.xlsx` 1-16 按新口径重定 health / strength / speed(护甲 / 抗性 / 暴击 / 奖励不变),三档目标同 2026-09-11:
    教学怪参照 1 级没加点的号 5 下打死、每下约 3% 气血;普通怪参照同级通用号 3 下、约 6%、速度 0.9 倍;
    16 号首领参照 30 级 3 人队 12 下、约 12%、速度持平。新值(血 / 力量 / 速度):1 号 370/14/20、2 号 360/14/21、
    3 号 1430/53/29、4 号 2010/63/34、5 号 2660/73/40、6 号 3140/82/45、7 号 3740/92/50、8 号 4400/106/58、
    9 号 5270/121/67、10 号 5780/135/75、11 号 6890/154/86、12 号 7230/168/94、13 号 8550/192/107、
    14 号 9190/216/121、15 号 10910/249/140、16 号 23850/365/110。
    推算:1 级没加点单人副本 1 约 10 回合胜、剩 60% 血;10~12 级通用单人副本 2 约 6~7 回合、剩 50%~60%;
    3 人队 25~28 级副本 3 约 7~8 回合、剩 50%~60%;单人 30 级打副本 3、10 级没加点打副本 2 会输。
  - 下文 2026-09-11 旧口径里 ① 的怪物数值与 ③ 的 PVP 推算作废,只留作对照。**这组怪物数值依赖尚未编译验证的
    加点百分比公式**:加点公式再调,怪物要按同一口径重算。
  相性 / 仙魔池没跟着改,它们相对属性点的价值比 2026-09-11 同比上调时**又倒挂了一次**(相性满投 50 点金相 = +2650 法伤,
  已高于灵力满投 425 点的 +918):要么把相性 / 仙魔也切到百分比公式,要么把它们的系数按新口径再缩一次,同样待拍板。
- **单点分配无反馈**(2026-09-13 公式性质):速度 / 防御每点分配约 +0.09、法力约 +0.31,分别连投 12 / 12 / 4 点才看到 +1
  (§3.1)。若玩家反馈"加点没生效":要么接受,要么在面板下发"下一点预计增量",要么调比例,待拍板。
