...
**可调整的顺序**:B3(序 5–7)与 B4(序 8–11)互不依赖,可整体对调以尽早开工 B5;对调后 `asset_op_ledger` 取 15、`profile_component` 取 16(G-03 规则不变)。B3b 只依赖 B2c 与 B3a-2,可晚于 B5c。B5d 可与 B6 并行编写,但合入仍串行。
