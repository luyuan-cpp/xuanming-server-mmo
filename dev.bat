...
if /I "%CMD%"=="obs"        goto :obs
if /I "%CMD%"=="obs-down"   goto :obs_down
...
echo     23. Obs         (docker-compose up: Grafana + Loki + Alloy log viewer)
echo     24. Obs down    (docker-compose down: Grafana + Loki + Alloy)
echo.
echo     0.  Exit
echo.
set /p "CHOICE=  Pick [0-24]: "
...
if "%CHOICE%"=="23" call :obs           & goto :menu_return
if "%CHOICE%"=="24" call :obs_down      & goto :menu_return
...
:: ================================================================
:obs
echo Starting log viewer stack (docker-compose: Grafana + Loki + Alloy)...
...
:: ================================================================
:obs_down
echo Stopping log viewer stack...
docker compose -f deploy\docker-compose.observability.yml down
if errorlevel 1 ( echo docker-compose down failed. & pause & exit /b 1 )
echo Log viewer stopped (Loki data volume kept; use "docker compose -f deploy\docker-compose.observability.yml down -v" to wipe).
exit /b 0
...
echo     obs            Start Grafana + Loki + Alloy log viewer (http://localhost:3000)
echo     obs-down       Stop the log viewer stack
