...
- sidecar 容器补 `securityContext`:uid/gid 10001、`runAsNonRoot`、`allowPrivilegeEscalation: false`、`readOnlyRootFilesystem: true`、capabilities drop ALL,与 `Dockerfile.cpp` 的 `USER 10001:10001` 同一约定。Alloy 的 HTTP 端点从 `127.0.0.1:12345` 改到 `0.0.0.0:12345` 并声明 `alloy-http` containerPort —— 不然 Pod 外抓不到 `/metrics`(`alloy_loki_source_file_*` / `loki_write_*`)和 `/-/reload`。
...
- **Loki 缺席时给警告**:`Apply-Zone` / `Apply-BattlePool` 在 apply 前打印一行 sidecar 提示(镜像名 + 「kind 需先把镜像载入节点」+「不想要就 `-NoCppLogSidecar`」),并在用集群内默认 Loki 时只读探一次 `kubectl -n <infra-ns> get svc loki --ignore-not-found`,查不到就 `Write-Warning`:部署会成功但一条 C++ 日志都查不到,先跑 `infra-up` 或改用 `-LokiPushUrl` / `-NoCppLogSidecar`。`-DryRun` 下跳过探测。sidecar 与 Loki 分属两条部署路径,这是 `zone-up`-only / `-SkipInfra` 的唯一坑,已同步写进 `deploy/k8s/AGENTS.md`。
- **契约测试**:`tools/scripts/tests/k8s_deploy_contract.tests.ps1` 新增 5 条(原生 sidecar 形态 + config-hash 注解、裸 `-` label 值、battle 无 zone 标签 / zone 的值带引号、`-NoCppLogSidecar` 产物洁净、日志清理循环 + sizeLimit),并更新两条 args 断言。**本轮未运行**(按 §10.1 由 Codex 跑)。
- **已验证到这一步**:4 组 dry-run(`infra-up` / `zone-up` / `zone-up -SceneOrchestrator agones` / `zone-up -NoCppLogSidecar`)退出码 0,所有 manifest 块经 python `yaml.safe_load_all` 解析通过(infra 12 块 / zone 6 / agones 7 / nosidecar 5);修复前 `infra-up` 的 battle sidecar ConfigMap 块是解析失败的。生成物逐项核对:`initContainers` 里的 `log-sidecar` 带 `restartPolicy: Always`、securityContext、`alloy-http` 端口;pod 模板带 12 位 config-hash 注解;`node-logs` 是 `emptyDir { sizeLimit: 2Gi }`;battle 的 ConfigMap 不带 `mmorpg.io/zone`,zone 的是加引号的值。真跑 `grafana/alloy:v1.10.0`(`--read-only`、`--user 10001:10001`、日志目录只读挂载)吃**生成出来的** `config.alloy`:`Dropped log messages at ...` 独立成条且 `level=warn`,muduo 行 `level=info/error`、时间戳 `+08:00`,`filename` 出现在 structured_metadata、不在 labels 里,该镜像里有 `Asia/Hong_Kong` 时区,只读根文件系统 + uid 10001 正常工作。清理循环在 `ubuntu:24.04`(`Dockerfile.cpp` 的运行时基镜像)实跑:12 个文件 → 只剩最近 8 个,`xargs -r` 存在。kind 集群端到端复测本轮进行中(上一轮修改前已通过一次:`service=scene`、时间戳、namespace/pod/zone 标签都对)。
- **未验证 / 未做**:没有在真实 zone / infra namespace 上 apply 过(只在临时命名空间和 dry-run 上验证);没有用真实 C++ 镜像跑过(用 alloy 镜像当「假节点」写 muduo 格式行做的验证);Agones Fleet 没有经过真实 Agones 准入校验(集群里没装 Agones controller,也就没校验);契约测试未运行。
