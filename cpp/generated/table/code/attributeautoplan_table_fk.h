...
inline const AttributePoolTable* GetAttributeAutoPlanPoolIdRow(const AttributeAutoPlanTable& row) {
...
inline const AttributePoolTable* GetAttributeAutoPlanPoolIdRow(uint32_t tableId) {
...
inline std::vector<const AttributeDimensionTable*> GetAttributeAutoPlanDimensionRows(const AttributeAutoPlanTable& row) {
...
inline std::vector<const AttributeDimensionTable*> GetAttributeAutoPlanDimensionRows(uint32_t tableId) {
    auto [row, _] = AttributeAutoPlanTableManager::Instance().FindByIdSilent(tableId);
    if (!row) return {};
...
inline const std::vector<const AttributeAutoPlanTable*>& FindAttributeAutoPlanRowsByPoolId(uint32_t key) {
