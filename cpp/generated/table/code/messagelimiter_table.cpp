...
void MessageLimiterTableManager::Load() {
    ...
    if (UseProtoBinaryTables()) {
        const std::string path = GetConfigDir() + "messagelimiter.pb";
        const auto contents = File2String(path);
        if (!snap->data.ParseFromString(contents)) {
            LOG_FATAL << "MessageLimiter binary parse failed: " << path;
        }
    } else {
        const std::string path = GetConfigDir() + "messagelimiter.json";
        const auto contents = File2String(path);
        if (const auto result = google::protobuf::util::JsonStringToMessage(contents.data(), &snap->data); !result.ok()) {
            LOG_FATAL << "MessageLimiter" << result.message().data();
        }
    }
...
std::pair<const MessageLimiterTable*, uint32_t> MessageLimiterTableManager::FindById(const uint32_t tableId) {
...
std::pair<const MessageLimiterTable*, uint32_t> MessageLimiterTableManager::FindByIdSilent(const uint32_t tableId) {
    const auto& snap = GetSnapshot();
    const auto it = snap.idMap.find(tableId);
    if (it == snap.idMap.end()) {
        return {nullptr, kInvalidTableId};
