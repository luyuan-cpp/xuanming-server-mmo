...
public:
    ...
    //
    //   ✅ 存 id,用的时候现查
    //   ❌ 把 const AttributeRuleTable* 存进成员、容器、闭包、协程帧
    //
...
#define LookupAttributeRuleOrReturnError(tableId) \
    ...
    do { if (!(attributeRuleRow)) { TableLookupLogMissing("AttributeRule", tableId, __FILE__, __LINE__); return attributeRuleResult; } } while(0)
...
#define LookupAttributeRuleAsOrReturnError(prefix, tableId) \
    ...
    do { if (!(prefix##AttributeRuleRow)) { TableLookupLogMissing("AttributeRule", tableId, __FILE__, __LINE__); return prefix##AttributeRuleResult; } } while(0)

#define LookupAttributeRuleOrReturn(tableId, customReturnValue) \
    const auto [attributeRuleRow, attributeRuleResult] = AttributeRuleTableManager::Instance().FindByIdSilent(tableId); \
    do { if (!(attributeRuleRow)) { TableLookupLogMissing("AttributeRule", tableId, __FILE__, __LINE__); return customReturnValue; } } while(0)

#define LookupAttributeRuleOrReturnVoid(tableId) \
    const auto [attributeRuleRow, attributeRuleResult] = AttributeRuleTableManager::Instance().FindByIdSilent(tableId); \
    do { if (!(attributeRuleRow)) { TableLookupLogMissing("AttributeRule", tableId, __FILE__, __LINE__); return; } } while(0)

#define LookupAttributeRuleOrContinue(tableId) \
    const auto [attributeRuleRow, attributeRuleResult] = AttributeRuleTableManager::Instance().FindByIdSilent(tableId); \
    do { if (!(attributeRuleRow)) { TableLookupLogMissing("AttributeRule", tableId, __FILE__, __LINE__); continue; } } while(0)

#define LookupAttributeRuleOrReturnFalse(tableId) \
    const auto [attributeRuleRow, attributeRuleResult] = AttributeRuleTableManager::Instance().FindByIdSilent(tableId); \
    do { if (!(attributeRuleRow)) { TableLookupLogMissing("AttributeRule", tableId, __FILE__, __LINE__); return false; } } while(0)
