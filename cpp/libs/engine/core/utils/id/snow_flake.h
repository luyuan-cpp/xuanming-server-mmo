...
public:
	...
	Guid Generate()
	{
		...
		//
		// ⚠️ guard 预置的这一次**必须与真实耗尽走同一条等待路径,绝不能改成"直接借
		// guard+1"**(2026-08-11 在 Go 同构实现上试过,同秒重启回归当场抓出重号):
		// 这次等待不是低效,而是**跨重启屏障的一部分** —— 它强迫真实时间流逝越过
		// guard 秒,于是本进程发出的号永远落在"墙钟已真实到达"的秒里;下一任(亚秒
		// 重启 / 拿回同 node_id)用 max(墙钟, Redis 水位) 当地板时才罩得住前任全部
		// 发号秒。借位会把号发到墙钟前面,而 Redis 水位(WriteSnowFlakeGuard 写的
		// 也是墙钟口径)同样罩不住它。代价只是"激活同一秒内就有人发号"时那一次
		// <1s 的等待,现实中几乎不可达(首个 item/tx/snapshot 发号远晚于节点激活)。
		if (!guardPending) {
