...
// 发布到不同表版本,这里把七张战斗表的解析结果做成一个短指纹随快照携带,
// match 比对全员一致、battle 开局时再与自身比对(不一致按配置 warn/enforce)。
//
// 算法:对七张已加载的 proto 表对象(Skill/Buff/Cooldown/SkillPermission/Dungeon/Monster/Item)
...
//
// 缓存:Current() 进程内只算一次(七张表全量序列化,scene 每次备战都算不划算)。
...
namespace turnbattle
{
	...
	public:
		...
		// 表未加载时七张表都是空对象,算出的是"空表指纹"。
		static std::string Compute();

		// 对给定的七个表对象计算(纯函数,单测直接构造表对象验证)。
		// Item 表 2026-09-17 加入:战斗内用药效果改读 ItemTable.battle_* 列后,
		// 两端 Item 表不同版本会让同一 item_table_id 的回血量不同,必须进指纹。
